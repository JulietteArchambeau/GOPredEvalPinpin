...
![Correlation matrix including the geographical coordinates (i.e. latitude and longitude), elevation and climatic variables at the location of the populations. PC1, PC2 and PC3 correspond to the first three PC axes of the PCA based on the genomic data (Figure S[-@fig-PCA]). MEM1, MEM2, MEM3 and MEM4 correspond to the first four distance-based Moran's eigenvector maps (dbMEM) calculated based on the geographical coordinates of the populations with the function `dbmem` of the `adespatial` R package v`r packageVersion("adespatial")` [@adespatial2022a].](../figs/ExploratoryAnalyses/CorrplotClimVariables.pdf){#fig-CorrplotClimVariables}
...
The 0.5% SNPs with the lowest empirical $p$-values were considered as outliers.
...
outliers <- list(RDA = readRDS(here::here("outputs/RDA/RDA_outliers.rds"))[[3]]$outliers %>% dplyr::filter(maha_meth==T) %>% pull(snp),
                 ...
                 GF = readRDS(here::here("outputs/GF/gf_outliers.rds"))$gf_raw$outliers_rank005,
...
To make genomic offset predictions at the location of the NFI plots, we used:
  ...
  - the average climates under the time period 1901-1950, which are expected to represent the past climates under which the populations evolved. The time period 1901-1950 is the same reference period as the one used to predict the genomic offset at the location of the populations.
...
To make genomic offset predictions at the location of the common gardens, we used:

  - the average climates under the time period 1901-1950, which are expected to represent the past climates under which the populations evolved. The time period 1901-1950 is the same reference period as the one used to predict the genomic offset at the location of the populations and the NFI plots.
...
Here is the mathematical model that we used to estimate the association between genomic offset (GO) predictions or climate transfer distances (CTD) and the proportion of dead trees in the populations, independently in the five common gardens:
...
![Regression coefficients $\beta_{H}$ standing for the association between mortality rates and genomic offset predictions or climatic transfer distances (CTD). The mean and 95\% credible intervals are shown.](../figs/ValidationCommonGarden/MortalityModels/beta_H.pdf){#fig-ValCGMortalityModels}

### Height models

Here is the mathematical model that we used to estimate the association between tree height and genomic offset predictions or climatic transfer distances (CTD), independently in the five common gardens:
...
\end{align*}
  ...
  - $X_{p}$ is the value of the variable of interest (GO or CTD) in the population $p$. We include a quadratic term for $X_p$ to allow for potential nonlinearity in the response, as done for instance in @fitzpatrick2021experimental.

![Regression coefficients $\beta_{X_1}$ standing for the association between tree height and genomic offset predictions or climatic transfer distances (CTD) in the five common gardens. The mean and 95\% credible intervals are shown.](../figs/ValidationCommonGarden/HeightModels/beta_X1_withoutconfounder.pdf){#fig-ValCGHeightModelsBetaX1}

![Regression coefficients $\beta_{X_2}$ standing for the association between tree height and genomic offset predictions or climatic transfer distances (CTD). The mean and 95\% credible intervals are shown.](../figs/ValidationCommonGarden/HeightModels/beta_X2_withoutconfounder.pdf){#fig-ValCGHeightModelsBetaX2}


![Estimated mean relationship between tree height and the genomic offset predictions or the climatic transfer distances in Asturias, Spain. Tree height and $X_p$ were mean-centered and divided by their standard deviation.](../figs/ValidationCommonGarden/HeightModels/ScatterPlotsPredictedRelationships_withoutconfounder_asturias.pdf){#fig-ScatterPlotsCGAsturias}

![Estimated mean relationship between tree height and the genomic offset predictions or the climatic transfer distances in Bordeaux, France. Tree height and $X_p$ were mean-centered and divided by their standard deviation.](../figs/ValidationCommonGarden/HeightModels/ScatterPlotsPredictedRelationships_withoutconfounder_bordeaux.pdf){#fig-ScatterPlotsCGBordeaux}

![Estimated mean relationship between tree height and the genomic offset predictions or the climatic transfer distances in Cáceres, Spain. Tree height and $X_p$ were mean-centered and divided by their standard deviation.](../figs/ValidationCommonGarden/HeightModels/ScatterPlotsPredictedRelationships_withoutconfounder_caceres.pdf){#fig-ScatterPlotsCGCaceres}

![Estimated mean relationship between tree height and the genomic offset predictions or the climatic transfer distances in Madrid, Spain. Tree height and $X_p$ were mean-centered and divided by their standard deviation.](../figs/ValidationCommonGarden/HeightModels/ScatterPlotsPredictedRelationships_withoutconfounder_madrid.pdf){#fig-ScatterPlotsCGMadrid}

![Estimated mean relationship between tree height and the genomic offset predictions or the climatic transfer distances in Fundão, Portugal. Tree height and $X_p$ were mean-centered and divided by their standard deviation.](../figs/ValidationCommonGarden/HeightModels/ScatterPlotsPredictedRelationships_withoutconfounder_portugal.pdf){#fig-ScatterPlotsCGFundao}
