...
**ClimateDT**: [Climate Downscaling Tool](https://www.ibbr.cnr.it/climate-dt/). A geo-web service providing a large number of climatic variables downscaled at 1-km resolution between 1901 and 2098. 

**CTD**: Climatic transfer distance. An index calculated independently for each climatic variable and which is equal to the absolute difference between the values of the climatic variable at the location of the populations and at the location of the common gardens.

**dbMEMs**: Distance-based Moran’s Eigenvector Maps. Orthogonal vectors calculated based on a geographic distance matrix and maximizing the spatial autocorrelation (measured by Moran’s coefficient) of the sampled locations. 
...
**GDM**: Generalized Dissimilarity Modelling. One of the methods used to generate GO predictions at the location of the populations, the common gardens and the NFI plots (section @sec-GOpred-GDM).
...
In the present study, we used `r length(unique(df$pop))` populations and `r length(unique(df$clon))` genotypes. There were between `r range(df_sum$count)[[1]]` and `r range(df_sum$count)[[2]]` genotypes per population, with an average of about `r mean(df_sum$count) %>% round(1)` genotypes per population.
...
We identified outlier SNPs with the GF algorithm following @fitzpatrick2021experimental and @capblancq2023common. GF models were fitted to each locus individually. We then compared the importance ($R^2$) value of each locus to the $R^2$ distribution of all SNPs by calculating empirical $p$-values, an approach described in @lotterhos2014evaluation. More specifically, the empirical $p$-value $\hat{p}$ of each locus $l$ is calculated as follows:

$$\hat{p}_l = 1 - (r_l / N)$$

with $r_l$ being the rank of the $R^2$ value of locus $l$ within the distribution of $R^2$ values of the all SNPs ($N$ SNPs). 

Note that selecting the SNPs based on the rank of their empirical $p$-value is equivalent to selecting them based on the rank of their $R^2$ value, i.e., the 0.5% of the SNPs with the highest $R^2$ should match the 0.5% of the SNPs with the lowest empirical $p$-value.

We ran three independent GF runs and identified the SNPs that were in the 0.5% SNPs with the lowest empirical $p$-values across the three runs. 
...
40 SNPs were identified as outliers in the three runs and were kept for the following analyses.
...
outliers <- list(RDA = readRDS(here::here("outputs/RDA/RDA_outliers.rds"))[[3]]$outliers %>% dplyr::filter(maha_meth==T) %>% pull(snp),
                 ...
                 GF = readRDS(here::here("outputs/GF/GFCommonOutliers_ThreeRuns_RawAlleleFrequencies.rds")),
...
Among the `r length(common_cand)` outlier SNPs, we identified outlier SNPs located on the same scaffold/contig (a scaffold/contig measuring approximately between 400 and 10,000 bp). If two (or more) outlier SNPs were located on the same scaffold/contig, we kept the SNP with the lowest $p$-value in the RDA.
...
![Candidate SNPs.](../figs/RDA/AdaptiveIndexMap_CandidateSNPs_SI.png){#fig-AImapCandidates}

![Control SNPs.](../figs/RDA/AdaptiveIndexMap_ControlSNPs_SI.png){#fig-AImapControl}

Spatial projection of the predicted current genomic composition through extrapolation of the RDA model across maritime pine range. Pixels with similar colors are expected to have a similar genetic composition.
:::

The RDA-based genetic indexes were estimated for both present and future climates across the maritime pine range, and for both control and candidate SNPs. The GO was then calculated for each pixel of the maritime pine range and corresponds to the difference between the genetic indexes calculated for present and future climates.


:::{#fig-RDAGOmapsPopLocations layout-nrow=2 fig-pos='H'}

![Candidate SNPs.](../figs/RDA/GOmaps_PopLocations_CandidateSNPs_SI.png){#fig-RDAGOmapsPopLocationsCandidates height=3in width=6in}

![Control SNPs.](../figs/RDA/GOmaps_PopLocations_ControlSNPs_SI.png){#fig-RDAGOmapsPopLocationsControl height=3in width=6in}

GO predictions with RDA at the location of the studied populations.
:::
...
![Candidate SNPs.](../figs/RDA/GOmap_cand_SI.png){#fig-RDAGOmapsCandidates height=3in width=6in}

![Control SNPs.](../figs/RDA/GOmap_control_SI.png){#fig-RDAGOmapsControl height=3in width=6in}

GO predictions with RDA projected across the maritime pine range using the period 1901-1950 as reference period (i.e. past climates under which the populations evolved) and the predictions of five GCMs (one per panel) under the period 2041-2060 for the future climates (SSP3.7-0).
:::
...
![Candidate SNPs.](../figs/GF/GOmaps_PopLocations_CandidateSNPs_SI.png){#fig-GFGOmapsPopLocationsCandidates}

![Control SNPs.](../figs/GF/GOmaps_PopLocations_ControlSNPs_SI.png){#fig-GFGOmapsPopLocationsControl}

GO predictions with GF at the location of the studied populations.
:::


## Latent factor mixed models (LFMM) {#sec-GOpred-LFMM}

![GO predictions with LFMM and all SNPs.](../figs/LFMM/GOMaps_PopLocations_AllSNPs_SI.png){#fig-LFMMGOmapsPopLocationsAllSNPs}

:::{#fig-LFMMGOmapsPopLocations layout-nrow=2 fig-pos='H'}

![Candidate SNPs.](../figs/LFMM/GOmaps_PopLocations_CandidateSNPs_SI.png){#fig-LFMMGOmapsPopLocationsCandidates height=3in width=6in}

![Control SNPs.](../figs/LFMM/GOmaps_PopLocations_ControlSNPs_SI.png){#fig-LFMMGOmapsPopLocationsControl height=3in width=6in}

GO predictions with LFMM at the location of the studied populations.
:::
...
![Candidate SNPs.](../figs/GDM/GOmaps_PopLocations_CandidateSNPs_SI.png){#fig-GDMGOmapsPopLocationsCandidates}

![Control SNPs.](../figs/GDM/GOmaps_PopLocations_ControlSNPs_SI.png){#fig-GDMGOmapsPopLocationsControl}

GO predictions with GDM at the location of the studied populations.
:::
...
with $N$ the number of selected climatic variables (in our case, five), $X_{ref,i,p}$ the mean value of the climatic variable $i$ across the reference period 1901-1950 for the population $p$ and $X_{fut,i,p}$ the predicted mean value of the climatic variable $i$ across the future period 2041-2070 for the population $p$.

![Linear association between GO predictions with RDA and the control SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/RDA/ScatterPlotEucliDistance_control.pdf)

![Linear association between GO predictions with RDA and the candidate SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/RDA/ScatterPlotEucliDistance_cand.pdf)

![Linear association between GO predictions with GF and the control SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/GF/ScatterPlotEucliDistance_control.pdf)

![Linear association between GO predictions with GF and the candidate SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/GF/ScatterPlotEucliDistance_cand.pdf)

![Linear association between GO predictions with LFMM and all SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/LFMM/ScatterPlotEucliDistance_AllSNPs.pdf)

![Linear association between GO predictions with LFMM and the control SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/LFMM/ScatterPlotEucliDistance_control.pdf)

![Linear association between GO predictions with LFMM and the candidate SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/LFMM/ScatterPlotEucliDistance_cand.pdf)

![Linear association between GO predictions with GDM and the control SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/GDM/ScatterPlotEucliDistance_control.pdf)

![Linear association between GO predictions with GDM and the candidate SNPs and the Euclidean climatic distances. Populations are colored based on their main gene pool.](../figs/GDM/ScatterPlotEucliDistance_cand.pdf)
...
Projections of the GO predictions outside of the climatic space covered by the sampled populations have to be taken with caution. The figures below (and Figure 4a in the main manuscript) show the climatic space covered by the NFI plots and the 34 sampled populations for the reference period 1901-1950.
...
![GO predictions with RDA and the candidate SNPs at the location of the NFI plots.](../figs/RDA/NFI_GOmap_CandidateSNPs_SI.png){#fig-RDAGOpredictionsNFIplotsCandidates}

![GO predictions with RDA and the control SNPs at the location of the NFI plots.](../figs/RDA/NFI_GOmap_ControlSNPs_SI.png){#fig-RDAGOpredictionsNFIplotsControl}
...
![GO predictions with GF and the control SNPs at the location of the NFI plots.](../figs/GF/NFI_GOmap_ControlSNPs_SI.png){#fig-RGFGOpredictionsNFIplotsControl}

### GO predictions with LFMM

![GO predictions with LFMM and all SNPs at the location of the NFI plots.](../figs/LFMM/NFI_GOmap_AllSNPs_SI.png){#fig-LFMMGOpredictionsNFIplotsAllSNPs}

![GO predictions with LFMM and the candidate SNPs at the location of the NFI plots.](../figs/LFMM/NFI_GOmap_CandidateSNPs_SI.png){#fig-LFMMGOpredictionsNFIplotsCandidates}

![GO predictions with LFMM and the control SNPs at the location of the NFI plots.](../figs/LFMM/NFI_GOmap_ControlSNPs_SI.png){#fig-LFMMGOpredictionsNFIplotsControl}

### GO predictions with GDM

![GO predictions with GDM and the candidate SNPs at the location of the NFI plots.](../figs/GDM/NFI_GOmap_CandidateSNPs_SI.png){#fig-GDMGOpredictionsNFIplotsCandidates}

![GO predictions with GDM and the control SNPs at the location of the NFI plots.](../figs/GDM/NFI_GOmap_ControlSNPs_SI.png){#fig-GDMGOpredictionsNFIplotsControl}
...
![Candidate SNPs.](../figs/RDA/GOmap_cand_asturias_SI.png){#fig-RDAGOmapsAsturiasCandidates}

![Control SNPs.](../figs/RDA/GOmap_control_asturias_SI.png){#fig-RDAGOmapsAsturiasControl}
...
![Candidate SNPs.](../figs/RDA/GOmap_cand_bordeaux_SI.png){#fig-RDAGOmapsBordeauxCandidates}

![Control SNPs.](../figs/RDA/GOmap_control_bordeaux_SI.png){#fig-RDAGOmapsBordeauxControl}
...
![Candidate SNPs.](../figs/RDA/GOmap_cand_caceres_SI.png){#fig-RDAGOmapsCaceresCandidates}

![Control SNPs.](../figs/RDA/GOmap_control_caceres_SI.png){#fig-RDAGOmapsCaceresControl}
...
![Candidate SNPs.](../figs/RDA/GOmap_cand_madrid_SI.png){#fig-RDAGOmapsMadridCandidates}

![Control SNPs.](../figs/RDA/GOmap_control_madrid_SI.png){#fig-RDAGOmapsMadridControl}
...
![Candidate SNPs.](../figs/RDA/GOmap_cand_portugal_SI.png){#fig-RDAGOmapsPortugalCandidates}

![Control SNPs.](../figs/RDA/GOmap_control_portugal_SI.png){#fig-RDAGOmapsPortugalControl}
...
![Candidate SNPs.](../figs/GF/GOmap_cand_asturias_SI.png){#fig-GFGOmapsAsturiasCandidates}

![Control SNPs.](../figs/GF/GOmap_control_asturias_SI.png){#fig-GFGOmapsAsturiasControl}
...
![Candidate SNPs.](../figs/GF/GOmap_cand_bordeaux_SI.png){#fig-GFGOmapsBordeauxCandidates}

![Control SNPs.](../figs/GF/GOmap_control_bordeaux_SI.png){#fig-GFGOmapsBordeauxControl}
...
![Candidate SNPs.](../figs/GF/GOmap_cand_caceres_SI.png){#fig-GFGOmapsCaceresCandidates}

![Control SNPs.](../figs/GF/GOmap_control_caceres_SI.png){#fig-GFGOmapsCaceresControl}
...
![Candidate SNPs.](../figs/GF/GOmap_cand_madrid_SI.png){#fig-GFGOmapsMadridCandidates}

![Control SNPs.](../figs/GF/GOmap_control_madrid_SI.png){#fig-GFGOmapsMadridControl}
...
![Candidate SNPs.](../figs/GF/GOmap_cand_portugal_SI.png){#fig-GFGOmapsPortugalCandidates}

![Control SNPs.](../figs/GF/GOmap_control_portugal_SI.png){#fig-GFGOmapsPortugalControl}
...
![Asturias (Spain)](../figs/LFMM/GOmap_allSNPs_asturias_SI.png){#fig-LFMMGOmapAllSNPsAsturias}

![Bordeaux (France)](../figs/LFMM/GOmap_allSNPs_bordeaux_SI.png){#fig-LFMMGOmapAllSNPsBordeaux}

![Cáceres (Spain)](../figs/LFMM/GOmap_allSNPs_caceres_SI.png){#fig-LFMMGOmapAllSNPsCaceres}

![Madrid (Spain)](../figs/LFMM/GOmap_allSNPs_madrid_SI.png){#fig-LFMMGOmapAllSNPsAsturias}

![Fundão (Portugal)](../figs/LFMM/GOmap_allSNPs_portugal_SI.png){#fig-LFMMGOmapAllSNPsPortugal}
...
![Candidate SNPs.](../figs/LFMM/GOmap_cand_asturias_SI.png){#fig-LFMMGOmapsAsturiasCandidates}

![Control SNPs.](../figs/LFMM/GOmap_control_asturias_SI.png){#fig-LFMMGOmapsAsturiasControl}
...
![Candidate SNPs.](../figs/LFMM/GOmap_cand_bordeaux_SI.png){#fig-LFMMGOmapsBordeauxCandidates}

![Control SNPs.](../figs/LFMM/GOmap_control_bordeaux_SI.png){#fig-LFMMGOmapsBordeauxControl}
...
![Candidate SNPs.](../figs/LFMM/GOmap_cand_caceres_SI.png){#fig-LFMMGOmapsCaceresCandidates}

![Control SNPs.](../figs/LFMM/GOmap_control_caceres_SI.png){#fig-LFMMGOmapsCaceresControl}
...
![Candidate SNPs.](../figs/LFMM/GOmap_cand_madrid_SI.png){#fig-LFMMGOmapsMadridCandidates}

![Control SNPs.](../figs/LFMM/GOmap_control_madrid_SI.png){#fig-LFMMGOmapsMadridControl}
...
![Candidate SNPs.](../figs/LFMM/GOmap_cand_portugal_SI.png){#fig-LFMMGOmapsPortugalCandidates}

![Control SNPs.](../figs/LFMM/GOmap_control_portugal_SI.png){#fig-LFMMGOmapsPortugalControl}
...
![Candidate SNPs.](../figs/GDM/GOmap_cand_asturias_SI.png){#fig-GDMGOmapsAsturiasCandidates}

![Control SNPs.](../figs/GDM/GOmap_control_asturias_SI.png){#fig-GDMGOmapsAsturiasControl}
...
![Candidate SNPs.](../figs/GDM/GOmap_cand_bordeaux_SI.png){#fig-GDMGOmapsBordeauxCandidates}

![Control SNPs.](../figs/GDM/GOmap_control_bordeaux_SI.png){#fig-GDMGOmapsBordeauxControl}
...
![Candidate SNPs.](../figs/GDM/GOmap_cand_caceres_SI.png){#fig-GDMGOmapsCaceresCandidates}

![Control SNPs.](../figs/GDM/GOmap_control_caceres_SI.png){#fig-GDMGOmapsCaceresControl}
...
![Candidate SNPs.](../figs/GDM/GOmap_cand_madrid_SI.png){#fig-GDMGOmapsMadridCandidates}

![Control SNPs.](../figs/GDM/GOmap_control_madrid_SI.png){#fig-GDMGOmapsMadridControl}
...
![Candidate SNPs.](../figs/GDM/GOmap_cand_portugal_SI.png){#fig-GDMGOmapsPortugalCandidates}

![Control SNPs.](../figs/GDM/GOmap_control_portugal_SI.png){#fig-GDMGOmapsPortugalControl}

GO predictions with GDM in the common garden of Fundão, Portugal (black asterisk).
:::


### Correlations between GO predictions and CTDs

![Correlations between GO predictions and CTDs in Asturias, Spain.](../figs/ValidationCommonGarden/Corrplot_GOpreds_CTDs_asturias.pdf){#fig-CorrPlotGOpredCTDsAsturias}

![Correlations between GO predictions and CTDs in Bordeaux, France.](../figs/ValidationCommonGarden/Corrplot_GOpreds_CTDs_bordeaux.pdf){#fig-CorrPlotGOpredCTDsBordeaux}

![Correlations between GO predictions and CTDs in Cáceres, Spain.](../figs/ValidationCommonGarden/Corrplot_GOpreds_CTDs_caceres.pdf){#fig-CorrPlotGOpredCTDsCaceres}

![Correlations between GO predictions and CTDs in Madrid, Spain.](../figs/ValidationCommonGarden/Corrplot_GOpreds_CTDs_madrid.pdf){#fig-CorrPlotGOpredCTDsMadrid}

![Correlations between GO predictions and CTDs in Fundão, Portugal.](../figs/ValidationCommonGarden/Corrplot_GOpreds_CTDs_portugal.pdf){#fig-CorrPlotGOpredCTDsFundao}
...
The graph below is the same as S[-@fig-RelativeClimaticDistances], except that ALT and ARM populations are colored in pink and the other populations are colored in light blue.
