...
In the present study, we use `r length(unique(df$pop))` populations and `r length(unique(df$clon))` genotypes. There are between `r range(df_sum$count)[[1]]` and `r range(df_sum$count)[[2]]` genotypes per population, with an average of about `r mean(df_sum$count) %>% round(1)` genotypes per population.
