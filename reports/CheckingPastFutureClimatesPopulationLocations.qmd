...
list_fut_clim <-  list(ADJ=list(name="adjusted for elevation",
                                code="ADJ",
                                data=readRDS(here("data/ClimaticData/MaritimePinePops/FutureClimatePopulationLocationsPointEstimates.rds"))[[1]]),
                       noADJ=list(name="NOT adjusted for elevation",
                                  code="noADJ",
                                  data=readRDS(here("data/ClimaticData/MaritimePinePops/FutureClimatePopulationLocationsPointEstimates.rds"))[[2]]))                    
...
<span style="color: orange;">**Interpretation:**</span> Point estimate values and climatic values extracted from rasters are very similar, thus suggesting that we can use them interchangeably.
...
df_ref_means <-  y$ref_means %>% 
  ...
  pivot_longer(cols=clim_var$variables,names_to="variable",values_to="mean_ref")
...
<span style="color: orange;">**Interpretation**</span> 
  ...
  - the predictions are quite consistent across GCMs, except for the mean temperature of the driest quarter (`bio9`) of the **OLB population from Central Spain**. Indeed, for this population, the IPSL-CM6A-LR, MPI-ESM1-2-HR and UKESM1-0-LL GCMs GCMs predict a larger increase in `bio9` in the future than the GFDL-ESM4 and MRI-ESM2-0 GCMs. See the tables below that show the climatic values for the OLB population under past and future climates, with or without adjusted data for elevation, and with climatic data from point estimates or extracted from rasters. 
...
> Future climatic value extracted from rasters (2041-2070, SSP3-7.0):
...
<span style="color: red;">**How can we explain so strong differences among GCMs for the OLB population?**</span>
...
```{r LoadFutureClimatePointEstimatesSentByMaurizio, warning=F, eval=F,echo=F}
...
```{r CheckingNAs, eval=F,echo=F}
# We check that there is no NAs:
sapply(fut_rast, function(x) sum(is.na(x)))
```

```{r MakeViolinPlots, fig.height=12,fig.width=14, results="hide", eval=F,echo=F}
# Violin plots
...
```{r ViolinPlotsFutureDataPointEstimatesVsPastClimatesPointEstimates, fig.height=12,fig.width=14, results="hide", eval=F,echo=F}
