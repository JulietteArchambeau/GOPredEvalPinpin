...
params <-  coefftab$term %>% unique() %>% str_subset("^beta|R_squared")
...
```{r HeightPredictionsM2, message=F,warning=F, fig.height=6, fig.width=6}
...
go_methods <- list(LFMM_all = "LFMM and all SNPs",
                   LFMM_cand = "LFMM and candidate SNPs",
                   LFMM_control = "LFMM and control SNPs",
                   RDA_cand = "RDA and candidate SNPs",
                   RDA_control = "RDA and control SNPs",
                   GDM_cand = "GDM and candidate SNPs",
                   GDM_control = "GDM and control SNPs",
                   GF_cand = "GF and candidate SNPs",
                   GF_control = "GF and control SNPs")

myplots <- lapply(unique(height_data$cg), function(site_i){

myplots <- lapply(names(go_methods), function(method_i){
...
# extract only mean values of mu
mu_mean_df <- mu_pred %>%
  dplyr::select(x,mu_mean) %>% 
  dplyr::distinct()
...
y_pred <- 
  ...
  gather(x, y, -Iter) %>%
  group_by(x) %>%
  mutate(pi_l = rethinking::PI(y, prob = 0.90)[1],
...
p2 <- p +
  geom_ribbon(data = y_pred,
                 mapping = aes(x=x, ymin=pi_l, ymax=pi_h), alpha = .1, fill='dodgerblue') +
...
# Title
title <- ggdraw() + 
  draw_label(paste0(cg_names[[site_i]]," - ",go_methods[[method_i]]),
             fontface = 'bold',
             x = 0, hjust = 0) +   
...
p2 <- p2 + labs(title = go_methods[[method_i]])

return(list(p2,p1_p2))

})


plot_grid(myplots[[1]][[1]] + labs(x=""),
          myplots[[2]][[1]] + labs(y="",x=""),
          myplots[[3]][[1]] + labs(y="",x=""),
          myplots[[4]][[1]] + labs(x=""),
          myplots[[5]][[1]] + labs(y="",x=""),
          myplots[[6]][[1]] + labs(y="",x=""),
          myplots[[7]][[1]],
          myplots[[8]][[1]] + labs(y=""),
          myplots[[9]][[1]] + labs(y=""),
          ncol=3) %>% 
ggsave(file=here(paste0("figs/ValidationCommonGarden/HeightModels/HeightPredictions_",site_i,"_M2.pdf")),
       device="pdf",
       height=12,
       width=12)


lapply(myplots, function(x) x[[2]]) %>% setNames(names(go_methods))

}) %>% setNames(unique(height_data$cg))


pdf(width = 12, height = 6, here("figs/ValidationCommonGarden/HeightModels/HeightPredictions_Allgraphs.pdf"))
lapply(myplots, function(x) x)
dev.off()
