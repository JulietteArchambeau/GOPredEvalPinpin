...
The response variable is counts of dead trees. To calculate those counts, we load the survival data in the common gardens, in which `0` corresponds to dead trees and `1` to survivors.
...
We plot the 95% credible intervals of $\beta_H$ (effect of the initial tree height) and $\beta_X$ (associaiton with the genomic offset predictions, i.e. effect of the potential maladaptation of the populations when transplanted in the new environment of the common gardens).
...
with $Y_i$ is the height of the individual $i$ in the population $p$ in the block $b$ and $X_{p}$ is the value of the variable of interest (GO or CTD) in the population $p$. We include a quadratic term for $X_p$ to allow for potential nonlinearity in the response, following @fitzpatrick2021experimental.
