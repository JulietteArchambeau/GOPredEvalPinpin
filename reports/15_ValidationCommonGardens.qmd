...
In this report, we aim to evaluate the genomic offset predictions from the different GEA methods (LFMM, GF, GDM and RDA) with phenotypic data from five clonal common gardens located in Spain (Asturias, Madrid, Cáceres), Portugal (Fundão) and France (Pierroton). For that, for each GEA method, we predicted the genomic offset of each population when transplanted in the environment of the common garden (so instead of predicting the genomic offset into future climates, we predict it in the new environment of the common garden, i.e. space-for-time approach). The rational is that populations with the highest genomic offset in each common garden are expected to have a lower relative fitness in the common gardens. In this report, we use tree height and mortality in five common gardens as proxies of fitness.
...
coeff_match <- list(beta_H="$\\beta_{H}$ estimates (effect of initial tree height)",
                    beta_X="Regression coefficients") #$\\beta_{X}$ estimates
...
with $Y_{ipb}$ is the height of the individual $i$ in the population $p$ in the block $b$ and $X_{p}$ is the value of the variable of interest (GO or CTD) in the population $p$. We include a quadratic term for $X_p$ to allow for potential nonlinearity in the response, following @fitzpatrick2021experimental.


```{r CompileHeightModel,message=F,warning=F}
...
coeff_match <- list(beta_X1="Regression coefficients $\\beta_{X_1}$",
...
# save in pdf and png
ggsave(p_allvar,
       file=here(paste0("figs/ValidationCommonGarden/HeightModels/",coeff,"_",confounder,"_SNPsandCTD.pdf")),
       device="pdf",
       height=9,
       width=12)

ggsave(p_allvar,
       file=here(paste0("figs/ValidationCommonGarden/HeightModels/",coeff,"_",confounder,"_SNPsandCTD.png")),
       height=9,
       width=12)
...
# save in pdf and png
ggsave(p_SNPs,
       file=here(paste0("figs/ValidationCommonGarden/HeightModels/",coeff,"_",confounder,"_SNPsets.pdf")),
       device="pdf",
       height=7,
       width=8)

ggsave(p_SNPs,
       file=here(paste0("figs/ValidationCommonGarden/HeightModels/",coeff,"_",confounder,"_SNPsets.png")),
       height=7,
       width=8)
...
ggsave(filename = here(paste0("figs/ValidationCommonGarden/HeightModels/ScatterPlotsPredictedRelationships_",confounder,"_",cg_i,".pdf")), device="pdf", width=12,height=7)
...
```{r CompileHeightModelConfounder, message=F,warning=F}
...
```{r PolyPlotsWithConfounder, fig.width=12, fig.height=7}
