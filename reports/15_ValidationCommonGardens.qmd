...
p <- coefftab %>% 
  ...
  mutate(method_input=factor(method_input, levels=c("All SNPs",
         ...
         cg=factor(cg, levels=c("Madrid, Spain (13 months)",
...
# ============================================
# Option 1 - same image, different image sizes
# ============================================

# loading the picture of a cloud
...
p_SNPs_1 <- p_SNPs + list_pngs[[1]]+ list_pngs[[2]]+ list_pngs[[3]]+ list_pngs[[4]]+ list_pngs[[5]]
...
# ============================================
# Option 2 - different images, same image size
# ============================================

df_png <- p %>% 
  dplyr::select(cg) %>%
  distinct %>% 
  dplyr::mutate(image = case_when(cg == "Asturias, Spain (37 months)" ~ "reports/cloud.png",
                                  cg == "Bordeaux, France (85 months)" ~ "reports/cloud.png",
                                  cg == "Cáceres, Spain (8 months)" ~ "reports/sun.png",
                                  cg == "Madrid, Spain (13 months)" ~ "reports/sun.png",
                                  cg == "Fundão, Portugal (27 months)" ~ 'reports/cloud_and_sun.png'))
...
png_cg = annotation_custom2(rasterGrob(readPNG(here(df_png[df_png$cg==cg_i,"image"])),interpolate=TRUE), 
                            ymin = -0.56,
                            ymax= -0.28,
                            xmin = 4.02,
                            xmax = 4.55, 
...
p_SNPs_2 <- p_SNPs + list_pngs[[1]]+ list_pngs[[2]]+ list_pngs[[3]]+ list_pngs[[4]]+ list_pngs[[5]]
...
# Old version of the function to generate the quadratic relationships between tree height and the GO predictions / CTD.
generate_poly_plots <- function(model_i){
...
```{r RunHeightModel3, eval=F}
lapply(sites, function(site_i){
...
# Model coefficients
 coeffs <- broom.mixed::tidyMCMC(mod,
                          pars=params_to_estimate,
                          droppars = NULL, 
                          robust = FALSE, # give mean and standard deviation
                          ess = F, 
    ...
    coeffs %>% saveRDS(file=here(paste0("outputs/ValidationCommonGarden/HeightModels/coeffs_M3/coefftab_M3_",site_i,"_",method_i,".rds")))
...
```{r TableCoeffM3}
# We merge all the coefficients in ien table
lapply(unique(height_data$cg), function(site_i){
lapply(unique(df$method), function(method_i){

readRDS(file=here(paste0("outputs/ValidationCommonGarden/HeightModels/coeffs_M3/coefftab_M3_",site_i,"_",method_i,".rds")))
...
coeffs %>% saveRDS(file=here(paste0("outputs/ValidationCommonGarden/HeightModels/coeffs_M4/coefftab_M4_",site_i,"_",method_i,".rds")))
...
readRDS(file=here(paste0("outputs/ValidationCommonGarden/HeightModels/coeffs_M4/coefftab_M4_",site_i,"_",method_i,".rds")))
