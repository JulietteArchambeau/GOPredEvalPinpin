...
plot_grid(plotlist = plots[c(1,3)], nrow=1) %>% 
  ggsave(filename = here("figs/GDM/PredictedClimaticDistancevsObservedGeneticDistancePlots_SI.pdf"), device="pdf", width=10, height=6)
...
# We save a figure for the Supplementary Information
plot_grid(plotlist = plots[c(1,3)], nrow=1) %>% 
  ggsave(filename = here("figs/GDM/PredictedvsObservedGeneticDistancePlots_SI.pdf"), device="pdf", width=10, height=6)
...
# =====================================================
if(x$set_code=="cand"){ 
  ggsave(here("figs/GDM/IsplinePlot_CandidateSNPs_SI.pdf"), p, width=13,height=9, device="pdf")
  
  } else if(x$set_code=="control"){
    ggsave(here("figs/GDM/IsplinePlot_ControlSNPs_SI.pdf"), p, width=13,height=9, device="pdf")
    }
...
# =====================================================
if(x$set_code=="cand"){ 
  ggsave(here("figs/GDM/GOMaps_PopLocations_CandidateSNPs_SI.pdf"), go_maps, width=10,height=6, device="pdf")
  ggsave(here("figs/GDM/GOMaps_PopLocations_CandidateSNPs_SI.png"), go_maps, width=10,height=6)
  
  } else if(x$set_code=="control"){
    ggsave(here("figs/GDM/GOMaps_PopLocations_ControlSNPs_SI.pdf"), go_maps, width=10,height=6, device="pdf")
    ggsave(here("figs/GDM/GOMaps_PopLocations_ControlSNPs_SI.png"), go_maps, width=10,height=6)
    }
...
This figure corresponds to Figure 6a in the main manuscript.
:::
...
df %>% saveRDS(here("outputs/GDM/go_pred_rasters_2.rds"))
```

```{r PlotMeanGOProjections, fig.height=8, fig.width=8, warning=F}
# Map options
# ===========
point_size = 2
x_limits = c(-10, 15)
...
df <- readRDS(here("outputs/GDM/go_pred_rasters_2.rds"))
...
p %>% ggsave(here("figs/GDM/GOmeanProjections_CandSNPs.pdf"),., width=8,height=8, device="pdf")
p %>% ggsave(here("figs/GDM/GOmeanProjections_CandSNPs.png"),., width=8,height=8)
p
```
...
if(x$set_code=="cand"){ 
  ...
  ggsave(here("figs/GDM/NFI_GOmap_CandidateSNPs_SI.pdf"), p_SI, width=8,height=8, device="pdf")
  ggsave(here("figs/GDM/NFI_GOmap_CandidateSNPs_SI.png"), p_SI, width=8,height=8)
  
  } else if(x$set_code=="control"){
    p_SI <- p + theme(plot.title = element_blank())
    ggsave(here("figs/GDM/NFI_GOmap_ControlSNPs_SI.pdf"), p_SI, width=8,height=8, device="pdf")
    ggsave(here("figs/GDM/NFI_GOmap_ControlSNPs_SI.png"), p_SI, width=8,height=8)
    }
...
```{r CorrelationGONFIPlots, fig.height=5,fig.width=5, results="hide"}
...
p <- lapply(snp_sets, function(x) {
 ...
 if(x$set_code=="cand") {
   ...
   p_SI %>% 
   ggsave(filename = here(paste0("figs/GDM/GOmap_",x$set_code,"_",cg_name,"_SI.pdf")), device = "pdf",width=5,height=5)
   
   p_SI %>% 
   ggsave(filename = here(paste0("figs/GDM/GOmap_",x$set_code,"_",cg_name,"_SI.png")),width=5,height=5)
   }
  ...
  if(x$set_code=="control") {
   ...
   p_SI %>% 
   ggsave(filename = here(paste0("figs/GDM/GOmap_",x$set_code,"_",cg_name,"_SI.pdf")), device = "pdf",width=7,height=5)
   
   p_SI %>% 
   ggsave(filename = here(paste0("figs/GDM/GOmap_",x$set_code,"_",cg_name,"_SI.png")),width=7,height=5)
   }
