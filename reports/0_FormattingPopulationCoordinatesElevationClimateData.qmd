...
We load the files sent by Maurizio Marchi with climatic variables extracted at the location of each population.

We generate some datasets with climatic information for the variables of interest and only for the CLONAPIN populations. We generate two files: one for the climatic data adjusted for elevation, and one for the climatic data not adjusted for elevation. In each file, we store a list with each element corresponding to a reference period (i.e. 1901-1950 and 1961-1990). And for each reference period, we generate a dataset with the climatic variables in columns averaged over the period considered and a dataset with four columns: the populations, the climatic variables and the mean and standard deviation of the climatic variables over the period considered.
...
saveRDS(here(paste0("data/ClimaticData/MaritimePinePops/PastClimatePopulationLocations_",adj,".rds")))
...
# Predictions of future climates for the period 2041-2070 with the GCM CMIP6 GFDL-ESM4
# ------------------------------------------------------------------------------------
...
tab <-   read_csv(here(paste0("data/ClimaticData/MaritimePinePops/ClimateDTfiles/Extraction/ClimateDT_cmip6_GFDL-ESM4_POPcoordinates_",adj,".csv")), 
  ...
  dplyr::select(pop,contains("ude"),contains("prc"), contains("tmx"), contains("tmn"))
...
```{r PredictionsFutureClimatesOld, eval=F, echo=F}
# Predictions of future climates for the period 2041-2070 with the GCM CMIP6 GFDL-ESM4
# ------------------------------------------------------------------------------------

lapply(list("ADJ","noADJ"), function(x){
  
  read_csv(here(paste0("data/ClimaticData/MaritimePinePops/ClimateDTfiles/Extraction/ClimateDT_cmip6_GFDL-ESM4_POPcoordinates_",x,".csv")), 
           show_col_types = FALSE) %>% 
