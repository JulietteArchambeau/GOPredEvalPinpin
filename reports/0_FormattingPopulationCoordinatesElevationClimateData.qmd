...
## Datasets in the DRYAD repository


We include in the DRYAD repository the annual climatic data extracted at the location of the 34 CLONAPIN populations between 1901 and 2021 (point estimates): file `ClimateDT_Populations_PastClimates.csv`.


```{r DryadRepoPastClimates}
  read_csv(here(paste0("data/ClimaticData/MaritimePinePops/ClimateDTfiles/Extraction/ClimateDT_cmip6_GFDL-ESM4_POPcoordinates_noADJ.csv")), show_col_types = FALSE) %>% 
  dplyr::rename(longitude=Longitude,
                latitude=Latitude,
                elevation=Elevation,
                year=Year,
  ...
  write_csv(here("data/DryadRepo/ClimateDT_Populations_PastClimates.csv"))
```

And we include in the DRYAD repository the predicted climatic data extracted from the rasters, which corresponds to averaged climatic values over the period 2041-2070 under the scenario  SSP3.7-0: file `ClimateDT_Populations_FutureClimates.csv`. 

```{r DryadRepoFutureClimates}
clim_df <- readRDS(here("data/ClimaticData/MaritimePinePops/ClimatePopulationLocationValuesExtractedFromRasters_FiveGCMs_2041-2070_SSP370.rds")) %>% 
  bind_rows() %>%
  dplyr::select(pop,gcm,everything()) %>% 
  write_csv(here("data/DryadRepo/ClimateDT_Populations_FutureClimates.csv"))
```
