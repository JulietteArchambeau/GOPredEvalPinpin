...
pop_coord %>% 
  ...
  write_csv(here("data/ClimaticData/Extraction/PopCoordinates_ClimateDT_PreExtractionTab.csv"))
```
...
# -------------------------------------------------------------------
clim <- read_csv(here("data/ClimaticData/MaritimePinePops/ClimateDT_cmip6_PopCoordinatesPreExtractionTab.csv"),
               show_col_types = FALSE) %>% 
...
clim_past <- clim %>% 
  ...
  dplyr::filter(Year<1951) %>%  # we keep the years btw 1901 and 1950
  dplyr::select(-Year) %>%
  group_by(pop) # group by population


# mean of the annual climatic values between 1901 and 1950
clim_past %>% 
  ...
  write_csv(here::here("data/DryadRepo/PopulationCoordinatesPastClimateInformation.csv"))

# mean and sd of the annual climatic values between 1901 and 1950
clim_past %>% 
...
bind_rows(x_mean,x_sd) %>% 
  ...
  saveRDS(here("data/ClimaticData/MaritimePinePops/MeanStandardDeviationPopulationClimaticVariables.rds"))
...
clim_fut %>% write_csv(here::here("data/DryadRepo/PopulationCoordinatesFutureClimateInformation.csv"))
```
