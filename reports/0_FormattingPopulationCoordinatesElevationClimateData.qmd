...
kable_mydf <- function(x, 
  ...
  kable() %>%  
  kable_styling(bootstrap_options = bootstrap_options, full_width = F, font_size = font_size) %>% 
...
pop_coord <- read_csv(here::here("data/PopulationData/coordinates_pinaster_populations_ade_phd.csv"),show_col_types = FALSE) %>% 
...
```{r VALpop}
pop_coord %>% filter(str_detect(code,"VAL")) %>% kable_mydf(boldfirstcolumn = F)
```
...
clonapin_pop_coord_juliette <- read_csv(here::here("data/PopulationData/coordinates_pinaster_populations_juliette_phd.csv"),
...
new_pop_coord <-read.delim(here::here("data/PopulationData/coordinates_pinaster_populations_21022023.txt")) %>% 
  dplyr::rename(code = CODE, 
                dataset = Data_set,
...
pop_coord %>% 
  dplyr::select(-POP) %>% 
  write_csv(here::here("data/PopulationData/population_information_pinaster_feb2023.csv"))
...
# -----------------------------------------------
pop_coord %>% 
  select(code,latitude,longitude,altitude) %>% # rm the column dataset
  write_csv(here::here("data/ClimaticData/Extraction/PopCoordinates_ClimateDT_PreExtractionTab.csv"))
```
