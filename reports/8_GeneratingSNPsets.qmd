...
# A set of SNPs with no missing data

Following the comments of one reviewer, we added a supplementary set of SNPs containing only SNPs without any missing data - to avoid imputation introduced biases.

```{r SNPsWitoutMIssingData}
geno <- read.csv(here("data/DryadRepo/FormattedFilteredGenomicData_AlleleCounts_withoutmaf.csv"))

control_without_nas <- geno %>% drop_na() %>% .[["snp_ID"]]
```
...
We save seven sets of SNPs: 
  ...
  - SNPs without NAs, i.e., `r length(control_without_nas)` SNPs.
...
snp_sets <- list(all_cand = list(set_code = "all_cand",
                 ...
                 all = list(set_code = "all",
                            set_name = paste0("All SNPs (", length(all_snps_id),")"),
                            set_snps = all_snps_id),
                 control_without_nas = list(set_code = "control_without_nas",
                                            set_name = paste0("SNPs without NAs (", length(control_without_nas),")"),
                                            set_snps = control_without_nas))
...
Meaning of the columns:
  ...
  18. `control_SNPs_without_NAs`: `TRUE` if the SNP had no mising data; `FALSE` otherwise.
...
listsnps <- read_csv2(here("data/GenomicData/SNPsInformation.csv"),show_col_types = FALSE) %>%  
  mutate(RDA_outliers = ifelse(snp_ID %in% list_candidates$`RDA (mahalanobis)`, TRUE, FALSE),
         ...
         control_SNPs_without_NAs = ifelse(snp_ID %in% control_without_nas, TRUE, FALSE))
...
nb_per_chr_df <- nb_per_chr_list[[1]] %>%
  ...
  full_join(nb_per_chr_list[[7]], by = "Chromosome / contig") %>%
  # Replace NA values with 0 across all numeric columns
  mutate(across(where(is.numeric), ~replace_na(., 0))) 
...
control_without_nas_contig <- nb_per_chr_df %>% filter(str_detect(`Chromosome / contig`, 'Contig')) %>% summarise(sum_column = sum(.[[8]], na.rm = TRUE)) %>% pull
...
Among the `r length(control_without_nas)` SNPs, `r nb_per_chr_df[nrow(nb_per_chr_df),8][[1]]` SNPs were mapped on the reference genome, including `r control_without_nas_contig` on contigs not assembled on the chromosomes.
...
legend_levels <-  c(snp_sets$all$set_name,
                    ...
                    #snp_sets$control_without_nas$set_name,
...
plot_chrm <- gen_coord %>%
  ...
  geom_segment(data = snp_sets$all_cand$gen_pos, aes(x = Absolute_POS, xend = Absolute_POS, y = 0.2, yend = 0.38, col = factor(snp_sets$all_cand$set_name, 
                                                                levels = legend_levels)), linewidth = 0.6, alpha=alpha_snp_sets) + 
  # # SNPs without missing data (green lines)
  # geom_segment(data = snp_sets$control_without_nas$gen_pos, aes(x = Absolute_POS, xend = Absolute_POS, y = 0.4, yend = 0.58, col = factor(snp_sets$control_without_nas$set_name, 
  #                                                               levels = legend_levels)), linewidth = 0.6, alpha=alpha_snp_sets) + 
  # Common candidate SNPs (green lines)
