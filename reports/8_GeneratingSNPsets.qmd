...
Following @fitzpatrick2021experimental and @capblancq2023common, GF models were fitted using:
  ...
  - the 'raw' or *uncorrected* allele frequencies, which we refer as *<span style="color: orange;">GF</span>*.
  
  - the *corrected* allele frequencies, i.e. the allele frequencies corrected for population structure = corrected for population reladness, which we refer as *<span style="color: orange;">GF-X</span>* 
...
list_candidates$"GF"=gf_out$gf_raw$outliers_rank005
...
common_cand <- list_candidates[c("RDA (mahalanobis)","GF","pRDA (mahalanobis)","GF-X","LFMM","BayPass")] %>% unlist() %>% .[duplicated(.)] %>% unique()
...
common_cand_RDA_LFMM_BayPass_GFraw <- list_candidates[c("RDA (mahalanobis)","pRDA (mahalanobis)","LFMM","BayPass", "GF")] %>% unlist() %>% .[duplicated(.)] %>% unique()
...
**Possible sets of candidate SNPs:**
  ...
  8. SNPs identified by at least two methods among *RDA*, *pRDA*, *LFMM*, *BayPass* and *GF* => <span style="color: OrangeRed;">`r common_cand_RDA_LFMM_BayPass_GFraw %>% length()` SNPs.</span>.
