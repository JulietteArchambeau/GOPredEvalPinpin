...
Below, we keep only the SNPs that have been identified with the Mahalanobis distance method.
...
Different options for the thresholds:
  ...
  - empirical $p$-value threshold: $p$-value < 0.005 or $p$-value < 0.001.
  
  - rank-based threshold. The 0.2% or 0.5% of the SNPs with the lowest empirical $p$-values (i.e. 19 or 49 SNPs in our study). As done in @capblancq2023common and @fitzpatrick2021experimental.
  
  - Bonferroni threshold.
  
In the following analyses, **we use the 0.5% SNPs with the lowest empirical $p$-values.**

**<span style="color: red;">Warning:</span>** Gradient Forest has only been used once as a genome scan [@capblancq2023common] and its ability to identify loci underlying local adaptation has still not been evaluated and confronted to other GEAs. This method does not provide a significance level for each SNPs that we can use to select the candidate SNPs and therefore we have to use empirical $p$-values, which may be less robust. Moreover, when we performed several runs of GF as a genome scan, we did not obtain the same exact set of candidate SNPs each time. A final observation that made us skeptical of GF is that no common candidates were found between GF-raw and GF-X (although we do not expect much overlap, it is still unexpected that there is no overlap at all). For these reasons, we have to be careful when using this method as a GEA and we may not want the selection of the candidates to rely on this method. 
...
p <- make_venndiagram(
  list("GF-raw"=gf_out$gf_raw$outliers_rank005,
       "GF-X"=gf_out$gf_X$outliers_rank005),
title="0.5% SNPs with the lowest empirical p-values")

pp <- make_venndiagram(
  list("GF-raw"=gf_out$gf_raw$outliers_rank002,
       "GF-X"=gf_out$gf_X$outliers_rank002),
title="0.2% SNPs with the lowest empirical p-values")

plot_grid(p,pp,nrow=1)

# we keep SNPs with empirical p-values < 0.001
list_candidates$"GF-raw"=gf_out$gf_raw$outliers_rank005
list_candidates$"GF-X"=gf_out$gf_X$outliers_rank005
```
...
make_venndiagram(list_candidates[c("RDA (mahalanobis)","GF-raw")],
                 caption = "GEA methods not correcting for population structure")
make_venndiagram(list_candidates[c("pRDA (mahalanobis)","GF-X","LFMM","BayPass")],
                 caption = "GEA methods correcting for population structure")


# Identifying SNPs identified by at least two GEA methods
common_cand_notcorrected <- list_candidates[c("RDA (mahalanobis)","GF-raw")] %>% unlist() %>% .[duplicated(.)] %>% unique()
common_cand_corrected <- list_candidates[c("pRDA (mahalanobis)","GF-X","LFMM","BayPass")] %>% unlist() %>% .[duplicated(.)] %>% unique()
common_cand <- list_candidates[c("RDA (mahalanobis)","GF-raw","pRDA (mahalanobis)","GF-X","LFMM","BayPass")] %>% unlist() %>% .[duplicated(.)] %>% unique()
...
common_cand_RDA_LFMM_BayPass_GFraw <- list_candidates[c("RDA (mahalanobis)","pRDA (mahalanobis)","LFMM","BayPass", "GF-raw")] %>% unlist() %>% .[duplicated(.)] %>% unique()
...
**Possible sets of candidate SNPs:**
  ...
  2. SNPs that were identified by at least one GEA method **not correcting for population structure**, namely *RDA* and *GF-raw* => <span style="color: OrangeRed;">`r common_cand_notcorrected %>% length()` SNPs.</span>
  
  3. SNPs that were identified by at least one GEA method **correcting for population structure**, namely *pRDA*, *GF-X*, *LFMM* and *BayPass* => <span style="color: OrangeRed;">`r common_cand_corrected %>% length()` SNPs.</span>
  ...
  8. SNPs identified by at least two methods among *RDA*, *pRDA*, *LFMM*, *BayPass* and *GF-raw* => <span style="color: OrangeRed;">`r common_cand_RDA_LFMM_BayPass_GFraw %>% length()` SNPs.</span>.
...
We will use the set with SNPs identified by at least two methods among *RDA*, *pRDA*, *LFMM*, *BayPass* and *GF-raw* (`r common_cand_RDA_LFMM_BayPass_GFraw %>% length()` SNPs). The rationale is that we aim to obtain a set of candidate SNPs that is enriched for loci involved in adaptation to climate, so we tried to find a good balance between not being too conservative (and choosing for instance a set of SNPs with SNPs in common across all methods) and not including too many false positives (eg, by selecting SNPs identified by at least by two methods). We also wanted to give the possibility to select candidates that can only be identified with methods correcting for population structure (pRDA, LFMM, BayPass) or candidates that can only be identified by methods not correcting for population structure (RDA and GF-raw). 

```{r VennDiagrammSelectedMethods,fig.height=7,fig.width=7}
make_venndiagram(list_candidates[c("RDA (mahalanobis)","pRDA (mahalanobis)","LFMM","BayPass","GF-raw")],
...
snppos %>% 
  dplyr::filter(snp_ID %in% common_cand_RDA_LFMM_BayPass_GFraw) %>% 
...
If we keep only one SNP per scaffold, we will have:
  ...
  - <span style="color: OrangeRed;">`r snppos %>% dplyr::filter(snp_ID %in% common_cand_RDA_LFMM_BayPass_GFraw) %>% pull(scaffold) %>% unique() %>% length()` candidate SNPs</span> identified by at least one GEA method among *RDA*, *pRDA*, *LFMM*, *BayPass* and *GF-X*.

  - <span style="color: OrangeRed;">`r snppos %>% dplyr::filter(snp_ID %in% common_cand_notcorrected) %>% pull(scaffold) %>% unique() %>% length()` candidate SNPs</span> identified by at least one GEA method not correcting for population structure (*RDA* and *GF-raw*).
  
  - <span style="color: OrangeRed;">`r snppos %>% dplyr::filter(snp_ID %in% common_cand_corrected) %>% pull(scaffold) %>% unique() %>% length()` candidate SNPs</span> identified by at least one GEA method correcting for population structure (*pRDA*, *GF-X*, *LFMM* and *BayPass*).
  
  
When some SNPs are on the same scaffold, we remove the SNPs with higher $p$-values in the RDA.

```{r rmSNPsSameGenomePosition}
snp_to_remove <- lapply(list(common_cand_RDA_LFMM_BayPass_GFraw,common_cand_corrected), function(set){
...
common_cand_RDA_LFMM_BayPass_GFraw <- setdiff(common_cand_RDA_LFMM_BayPass_GFraw, snp_to_remove[[1]])
...
nb_snps <- common_cand_RDA_LFMM_BayPass_GFraw %>% length()
...
snp_sets <- list(candidate = list(set_code = "cand",
                                  set_name = "Candidate SNPs",
                                  set_snps = common_cand_RDA_LFMM_BayPass_GFraw),
                 candidate_corrected = list(set_code = "cand_corrected",
