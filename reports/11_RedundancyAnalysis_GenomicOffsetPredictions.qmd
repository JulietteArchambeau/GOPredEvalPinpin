...
```{r GenerateGOMapsWithRasters, message=F, results="hide", warning=F, eval=T}
# Map options
# ===========
point_size = 2
x_limits = c(-10, 15)
...
go_dfs <- lapply(names(clim_dfs$clim_pred),function(gcm){
  rasterToPoints(go_rasters[[gcm]]$go_global_proj) %>% as_tibble()}) %>% 
  setNames(names(clim_dfs$clim_pred))

go_limits <- go_dfs %>% lapply(function(x) x %>% pull(global_offset)) %>% unlist() %>%
  range()
