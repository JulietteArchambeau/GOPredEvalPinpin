...
# Candidate SNPs
p <- ai_maps[[1]] + theme(plot.title = element_blank())
ggsave(here("figs/RDA/AdaptiveIndexMap_CandidateSNPs_SI.png"), p, width=9,height=4.9)
ggsave(here("figs/RDA/AdaptiveIndexMap_CandidateSNPs_SI.pdf"), p, width=9,height=4.9)

# Control SNPs
p <- ai_maps[[3]] + theme(plot.title = element_blank())
ggsave(here("figs/RDA/AdaptiveIndexMap_ControlSNPs_SI.png"), p, width=9,height=4.9)
ggsave(here("figs/RDA/AdaptiveIndexMap_ControlSNPs_SI.pdf"), p, width=9,height=4.9)
...
if(x$set_code=="cand"){ 
  ...
  ggsave(here("figs/RDA/GOMaps_PopLocations_CandidateSNPs_SI.png"), go_maps, width=10,height=6)
  ...
  ggsave(here("figs/RDA/GOMaps_PopLocations_ControlSNPs_SI.png"), go_maps, width=10,height=6)
}
...
go_rasters <- snp_sets %>% lapply(function(snp_set){
  ...
  pred_GO_rasters(snp_set,clim_var,K=2,gcm)}) %>% 
    setNames(names(clim_dfs$clim_pred)) %>% 
    saveRDS(file=here(paste0("outputs/RDA/go_rasters_",snp_set$set_code,".rds")))
  })
...
go_rasters <-readRDS(file=here(paste0("outputs/RDA/go_rasters_",snp_sets[[x]]$set_code,".rds")))  

rasterToPoints(go_rasters[[gcm]]$go_global_proj) %>% as_tibble() %>% pull(global_offset)
...
go_dfs <- rasterToPoints(go_rasters[[gcm]]$go_global_proj) %>% as_tibble()
...
ggsave(here(paste0("figs/RDA/GOMap_",x,"_SI.pdf")), go_maps, width=11,height=7, device="pdf")
ggsave(here(paste0("figs/RDA/GOMap_",x,"_SI.png")), go_maps, width=11,height=7)
...
This figure is included in the Supplementary Information.

![](../figs/RDA/GOMap_cand_SI.png)
...
![](../figs/RDA/GOMap_control_SI.png)

> Candidate SNPs with correction for population structure

i.e., SNPs identified by at least one GEA method correcting for population structure (*pRDA*, *LFMM* and *BayPass*).

![](../figs/RDA/GOMap_cand_corrected_SI.png)
...
if(x$set_code=="cand"){ 
  ...
  ggsave(here("figs/RDA/NFI_GOmap_CandidateSNPs_SI.pdf"), p_SI, width=6,height=6, device="pdf")
  ggsave(here("figs/RDA/NFI_GOmap_CandidateSNPs_SI.png"), p_SI, width=6,height=6)
  
  } else if(x$set_code=="control"){
    p_SI <- p + theme(plot.title = element_blank())
    ggsave(here("figs/RDA/NFI_GOmap_ControlSNPs_SI.pdf"), p_SI, width=6,height=6, device="pdf")
    ggsave(here("figs/RDA/NFI_GOmap_ControlSNPs_SI.png"), p_SI, width=6,height=6)
    }
...
p <- lapply(snp_sets, function(x) {
 ...
 if(x$set_code=="cand") {
   ...
   p_SI %>% 
   ggsave(filename = here(paste0("figs/RDA/GOmap_",x$set_code,"_",cg_name,"_SI.png")), width=5,height=5)
  ...
  if(x$set_code=="control") {
   ...
   p_SI %>% 
   ggsave(filename = here(paste0("figs/RDA/GOmap_",x$set_code,"_",cg_name,"_SI.png")), width=7,height=5)
   }
