...
We generate a map with the mean of genomic offset predictions across GCMs for the three sets of candidate SNPs.

```{r PreparationDatasetMeanGOprojectionAllCandidateSNPs, eval=F}
cand_set_names <- snp_sets[1:3] %>%  names()

lapply(cand_set_names, function(set_i){
  
  go_rasters <-readRDS(file=here(paste0("outputs/RDA/go_rasters_",set_i,"_",rda_method,"_",popstructure_correction,".rds")))
  ...
  saveRDS(go_df_mean, file=here(paste0("outputs/RDA/go_df_mean_",set_i,"_",rda_method,"_",popstructure_correction,".rds")))
})
```

```{r MeanGOprojectionCandidateSNPs, fig.height=6, fig.width=6}
cand_set_names <- snp_sets[1:3] %>%  names()

lapply(cand_set_names, function(set_i){
  
  go_df_mean <- readRDS(file=here(paste0("outputs/RDA/go_df_mean_",set_i,"_",rda_method,"_",popstructure_correction,".rds")))
  ...
  ggsave(here(paste0("figs/RDA/GOmeanProjections_",set_i,"_",rda_method,"_",
...
lapply(snp_sets, function(x) {
  ...
  ggsave(here(paste0("figs/RDA/NFI_GOmap_",x$set_code,"_PS",correction,"_",rda_method,".pdf")), p_notitle, width=6,height=6, device="pdf")
  ggsave(here(paste0("figs/RDA/NFI_GOmap_",x$set_code,"_PS",correction,"_",rda_method,".png")), p_notitle, width=6,height=6)
