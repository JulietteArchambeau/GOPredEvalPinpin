...
# We had a set with all SNPs to the list
snp_sets$all <- list(set_code = "all",
                     set_name = "All SNPs",
                     set_snps = names(geno))
```
...
We may project the genetic composition across the landscape as estimated by the RDA models.

```{r ProjectingAdaptiveIndex, eval=F}
# SNP set
selected_snp_set <- "cand"
...
# Save the map
ggmap %>% ggsave(here(paste0("figs/RDA/AdaptiveIndexMap_",selected_snp_set,"_",rda_method,"_PS",pop_structure,".pdf")), ., width=10,height=6)
...
high_go_pops <- make_high_go_pop_maps(pop_coord=pop_coord,
                                      list_go = snp_sets$cand$go_uncorrected,
                                      ggtitle="Candidate SNPs - RDA uncorrected",
...
go_rasters <- snp_sets %>% lapply(function(snp_set){
  ...
  pred_GO_rasters(snp_set,clim_var,K=2,gcm, method = rda_method)}) %>% 
    setNames(names(clim_dfs$clim_pred)) %>% 
    saveRDS(file=here(paste0("outputs/RDA/go_rasters_",snp_set$set_code,"_",rda_method,".rds")))
  })
...
go_rasters <-readRDS(file=here(paste0("outputs/RDA/go_rasters_",snp_sets[[x]]$set_code,"_",rda_method,".rds")))  
...
ggsave(here(paste0("figs/RDA/GOMap_",x,"_",rda_method,".pdf")), go_maps, width=11,height=7, device="pdf")
ggsave(here(paste0("figs/RDA/GOMap_",x,"_",rda_method,".png")), go_maps, width=11,height=7)
...
![](../figs/RDA/GOMap_cand_predict.png)
...
<!-- ![](../figs/RDA/GOMap_control_predict.png) -->

> All SNPs

<!-- ![](../figs/RDA/GOMap_all_predict.png) -->
...
<!-- ![](../figs/RDA/GOMap_cand_corrected_predict.png) -->
...
```{r CheckingCorrelationsPredictionsPointEstimatesVSRasters, eval=F}
...
if(x$set_code=="cand"){ 
  ...
  ggsave(here(paste0("figs/RDA/NFI_GOmap_CandidateSNPs_PS",correction,"_",rda_method,".pdf")), p_SI, width=6,height=6, device="pdf")
  ggsave(here(paste0("figs/RDA/NFI_GOmap_CandidateSNPs_PS",correction,"_",rda_method,".png")), p_SI, width=6,height=6)
