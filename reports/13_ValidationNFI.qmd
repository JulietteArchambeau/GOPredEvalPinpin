...
clim_dist <- clim_dist %>% 
  ...
  mutate(method = "CD",
         ...
         snp_set_name = case_when(snp_set_code == "bio1" ~ "Mean annual temperature (bio1, °C)",
                                  ...
                                  snp_set_code == "EucliDist" ~ "Euclidean climatic distance"),
...
lapply(unique(df$method_snp_set_code), function(method_snp_set_i){
  ...
  bind_rows() %>% 
  mutate(snp_set_name = factor(snp_set_name, levels = c(unique(coefftab$snp_set_name)[[6]],
...
coeff_match <- list(beta_GO=list("Regression coefficients $\\beta_{X}$",
                                 c(0.4,0.85),
                                 c(-0.2,0.4)), 
