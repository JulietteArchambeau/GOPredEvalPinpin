...
Mortality rates in natural populations are considered as a proxy of the absolute fitness of the populations. A positive association between GO predictions and the mortality rates would suggest that GO captures maladaptation in the face of demographic complexity (i.e., the effects of other processes on spatial variance in allele frequencies, such as expansion history and gene flow) as well as the genetic architecture of climate adaptation (polygenic trait architectures, G×E interactions, nonadditive genetic variance).
...
\begin{align*}
m_i &\sim \text{Binomial}(N_i,p_i)\\
\text{log}(-\text{log}(1-p_i)) &= \beta_{0,c} +  \beta_{C} C_i + \beta_{DBH} DBH_i + \beta_{C*DBH} C_i DBH_i + \beta_{GO} GO_i + \text{log}(\Delta_i) \\
\end{align*}
...
list_snps$LFMM <- readRDS(file=here("outputs/LFMM/go_predictions_snpsets.rds"))[[x$set_code]][["go_nfi"]]
...
```{r MortalityPredictions, message=F,warning=F, fig.height=12, fig.width=12, eval=T}
############################################################################################
# Visualizing the relationships between GO predictions and mortality probability predictions
############################################################################################

myplots <- lapply(unique(df$method_snp_set_code), function(method_snp_set_code_i){
...
tibble("Method" = df %>% filter(method_snp_set_code == method_snp_set_code_i) %>% .[["method_snp_set_name"]] %>% unique(),
       "Country"=country_i,
         "p(GO=-1)"=paste0(p_pred_sub$p_mean[p_pred_sub$x==-1], " [",
                       p_pred_sub$hpdi_l[p_pred_sub$x==-1],";",
                       p_pred_sub$hpdi_h[p_pred_sub$x==-1],"]"),
       "p(GO=0)"=paste0(p_pred_sub$p_mean[p_pred_sub$x==0], " [",
...
```{r IntervalPlotsM62, fig.height=5, fig.width=5, eval=T, results="hide"}
coefftab <- readRDS(file=here("outputs/ValidationNFI/ModelM62_coefftab.rds"))
  
coeff_match <- list(beta_GO=list("Regression coefficients $\\beta_{GO}$",
                                 c(0.73,0.825)), #"$\\beta_{GO}$ estimates",
                    beta_C=list("Regression coefficients $\\beta_C$",
                                c(0.73,0.2)),
                    beta_DBH=list("Regression coefficients $\\beta_{DBH}$",
                                  c(0.73,0.2)),
                    beta_C_DBH=list("Regression coefficients $\\beta_{C*DBH}$",
                                    c(0.73,0.8)))
...
p <- coefftab %>% 
  filter(term==coeff) %>% 
  # mutate(snp_set_name=factor(snp_set_name, levels=c("All SNPs",
  #                                                   "Candidate SNPs",
  #                                                   "Control SNPs"))) %>%
    ggplot(aes(x = method,
               y = median,
...
**Comment:** Estimating these parameters is computationally demanding as one value is estimated for each plot. So I estimated each parameter in independent model runs (otherwise, it was too heavy for my laptop).
...
We estimate these parameters for GO predictions with GF and the candidate SNPs only.

```{r RunMortalityModelM62WithPredictions, eval=F}
params_to_estimate <- c("p", "p_annual", "p_annual_go", "nb_dead_pred", "nb_dead_res")

# Running the model for GO predictions from GF and the candidate SNPs
method_i <- "GF (cand SNPs)"
...
saveRDS(sub_data, file = here(paste0("outputs/ValidationNFI/StanModels/M62_with_predictions/subdata_M62_with_predictions_",unique(sub_data$method_type),"_",unique(sub_data$method_input),".rds")))
...
saveRDS(stanlist, file = here(paste0("outputs/ValidationNFI/StanModels/M62_with_predictions/stanlist_M62_with_predictions_",unique(sub_data$method_type),"_",unique(sub_data$method_input),".rds")))

lapply(params_to_estimate, function(x){
  
# Running the Stan model
mod <- sampling(stancode, data = stanlist, iter = 2000, chains = 4, cores = 4,init=0, save_warmup = FALSE, include = TRUE, pars=x)

# Extract the coefficient of interest
mod %>% broom.mixed::tidyMCMC(pars=x,
                              droppars = NULL, 
                              robust = TRUE,  # give median and mean absolute deviation (= avg distance btw each data point and the mean)
                              ess = T, # effective sample size estimates
                              rhat = T, # Rhat estimates
  ...
  saveRDS(file=here(paste0("outputs/ValidationNFI/StanModels/M62_with_predictions/",x,"_",unique(sub_data$method_type),"_",unique(sub_data$method_input),".rds")))
...
coeffs <- readRDS(file=here(paste0("outputs/ValidationNFI/StanModels/M62_with_predictions/",x,"_",method_type,"_",method_input,".rds")))
  
data <- readRDS(file=here(paste0("outputs/ValidationNFI/StanModels/M62_with_predictions/subdata_M62_with_predictions_",method_type,"_",method_input,".rds"))) %>% 
  dplyr::select(plotcode, longitude, latitude) %>% 
  bind_cols(coeffs) 
...
```{r CompileMortalityModelM62WithPredictionsWithoutGO, message=F, warning=F, eval=F}
# Stan code
...
Here is an improved version of your comments, enhancing clarity and ensuring the English is precise:
...
```{r CalculateCorrelationsModelM62WithPredictionsWithoutGO}
corrtab <- lapply(unique(df$method_snp_set_code), function(method_snp_set_i){
  ...
  tibble(correlation = cor(sub_data$res_mortality_median, sub_data$GO),
         rank_correlation = cor(sub_data$rank_res_mortality_median, sub_data$GO_rank),
...
correlation_types <- c("correlation","rank_correlation")

lapply(correlation_types, function(coeff){
  
  p <- corrtab %>% 
    pivot_longer(values_to = "correlation_value", names_to = "correlation_type", cols = correlation_types) %>% 
    filter(correlation_type==coeff) %>% 
    ggplot(aes(x = method,
               y = correlation_value,
               color = snp_set_name,
               shape = snp_set_name)) +
    geom_rect(inherit.aes=FALSE, aes(xmin=-Inf, xmax=Inf, ymin=0, ymax=Inf), color="transparent", fill="green", alpha=0.01) + 
    geom_hline(yintercept = 0,color="gray") +
    geom_point(position = position_dodge(width = .4), size=3) +
    {if(coeff=="correlation") ylab("Correlation value") } + 
    {if(coeff=="rank_correlation") ylab("Rank correlation value") } + 
    xlab("") +
    ylim(c(-0.2,0.3)) +
    scale_color_manual(values=colors_coeff) +
    scale_shape_manual(values = c(rep(16,6),rep(17,3))) +
