...
`r nrow(list_geno$list_withoutmaf$tab[list_geno$list_withoutmaf$tab$na_freq<20,])` SNPs have less than 20% missing data and `r nrow(list_geno$list_withoutmaf$tab[list_geno$list_withoutmaf$tab$na_freq<15,])` have less than 15% missing data.

**<span style="color: red;">We keep SNPs with less than 20% missing data and so there are `r nrow(list_geno$list_withoutmaf$df)` SNPs left.</span>**
...
for(i in 1:length(list_geno)){
  ...
  write_csv(paste0("data/DryadRepo/FormattedFilteredGenomicData_AlleleCounts_",specMAF,".csv"),
            append=FALSE)
}
...
df_freq %>% 
  write_csv(paste0("data/DryadRepo/FormattedFilteredGenomicData_AlleleFrequencies_withoutmaf.csv"),
            append=FALSE)
...
list_geno[[i]]$df %>% 
  ...
  write_csv(paste0("data/DryadRepo/ListSNPs_",specMAF,".csv"),append=FALSE)
}
```
...
df <- read.csv("data/DryadRepo/FormattedFilteredGenomicData_AlleleCounts_withoutmaf.csv", row.names = 1) 
...
for(i in 1:length(list_geno)){
  ...
  list_geno[[i]]$df %>% 
  rownames_to_column(var="snp_ID") %>% 
  write_csv(paste0("data/DryadRepo/ImputedGenomicData_AlleleCounts_",specMAF,".csv"),
            append=FALSE)
}
```
...
for(i in 1:length(list_geno)){
  ...
  list_geno[[i]]$df %>% 
  write_csv(paste0("data/DryadRepo/ImputedGenomicData_AlleleFrequencies_",specMAF,".csv"),
            append=FALSE)
}
