...
**<span style="color: red;">In the dataset without MAF, there are `r nrow(list_geno$list_nomaf$df)` SNPs left.</span>** 
...
`r nrow(list_geno$list_nomaf$tab[list_geno$list_nomaf$tab$na_freq<20,])` SNPs have the percentage of missing data lower than 20% and `r nrow(list_geno$list_nomaf$tab[list_geno$list_nomaf$tab$na_freq<15,])` lower than 15%.

**<span style="color: red;">We keep SNPs with missing data lower than 20% and so there are `r nrow(list_geno$list_nomaf$df)` SNPs left.</span>**
...
list_geno$list_nomaf$df[1:10,1:8]  %>% kable_mydf(boldfirstcolumn = F, font_size = 11)
```
