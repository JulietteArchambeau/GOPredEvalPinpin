...
**<span style="color: red;">`r nrow(tab[tab$MAF<1,])` SNPs have MAF < 1%. In the dataset without MAF, we remove the SNPs with MAF < 1%: there are `r nrow(list_geno$list_nomaf$df) SNPs left.</span>** 

```{r RmMissingDataMAF}
list_geno <- list(list_maf=list(df = df),
                  list_nomaf=list(df = df[!(row.names(df) %in% tab[tab$MAF<1,]$snp),]))
```
...
`r nrow(list_geno$list_nomaf$tab[list_geno$list_nomaf$tab$na_freq<20,])` SNPs have less than 20% missing data and `r nrow(list_geno$list_nomaf$tab[list_geno$list_nomaf$tab$na_freq<15,])` have less than 15% missing data.

**<span style="color: red;">We keep SNPs with less than 20% missing data and so there are `r nrow(list_geno$list_nomaf$df)` SNPs left.</span>**


```{r PlotMissingDataPerSNP, fig.height=4,fig.width=6}
plot(list_geno$list_nomaf$tab$id,list_geno$list_nomaf$tab$na_freq,
     pch=20,
     xlab="SNP number",
...
tdf <- list_geno$list_maf$df %>% t() %>% as.data.frame() 
...
snp_match_1 <- read.csv("data/DryadRepo/SnpCodesMatching.csv") %>% 
  set_colnames(paste0(colnames(.),"_SNP1")) %>% 
  rename(SNP1 = snp_ID_SNP1)

snp_match_2 <- read.csv("data/DryadRepo/SnpCodesMatching.csv") %>% 
...
# snpcor %>% write_csv("data/GenomicData/CorrelatedSNPs.csv",append=FALSE) 
```
...
snpcor <- read.csv("data/GenomicData/CorrelatedSNPs.csv")
```


### SNP positions on the genome

This part aimed at understanding how the information about SNP position on the genome is stored in the excel file `genome_position.xlsx` (sent by Santi the 14/02/2023). In this excel file, the sheet `base_table` provides the information about the SNP positions on the genome. The position of the SNPs on the genome is given by the columns `scaffold` and `genome_position`. However, it is important to note that some SNPs have **alternative names**, referring to **different positions** on the genome. The alternative names are given in the column `annotation`. 

```{r SNPpositionInformationSheetBaseTable}
# we load the dataset with information about the SNP position on the genome
# -------------------------------------------------------------------------

snppos <- read_excel("data/GenomicData/genome_position.xlsx", sheet = "base_table") %>% 
...
sheet2 <- read_excel("data/GenomicData/genome_position.xlsx", sheet = "correlated_SNPs") %>% 
...
<span style="color: red;">For the pairs of SNPs located on the same scaffold, we remove one of the two SNPs of the pair.</span>

```{r ExtractCorrelatedSNPsToRemove, message=F}
# We extract the SNPs that we are going to remove
corrSNPs_to_remove <- read_excel("data/GenomicData/genome_position.xlsx", sheet = "correlated_SNPs") %>% 
...
We are going to remove **`r length(corrSNPs_to_remove)` SNPs.**

```{r RemoveCorrelatedSNPs}
for(i in 1:2) list_geno[[i]]$df <- list_geno[[i]]$df %>% dplyr::filter(!(rownames(.) %in% corrSNPs_to_remove)) 
```

<span style="color: red;">In the genomic dataset with MAF, **`r list_geno$list_maf$df %>% nrow()` SNPs left**.
In the genomic dataset without MAF, **`r list_geno$list_nomaf$df %>% nrow()` SNPs left**.</span>
...
for(i in 1:length(list_geno)){
  ...
  write_csv(paste0("data/DryadRepo/FormattedFilteredGenomicData_AlleleCounts_",
                   specMAF,"_",
                   ncol(list_geno[[i]]$df),"clones_",nrow(list_geno[[i]]$df),"snps.csv"),
            append=FALSE)
}
...
`r sum(is.na(list_geno$list_maf$df))` missing data in the dataset with MAF, and `r sum(is.na(list_geno$list_nomaf$df))` missing data in the dataset without MAF.
...
df_freq <- compute_allele_freq(list_geno$list_nomaf$df) 

df_freq %>% 
  write_csv(paste0("data/DryadRepo/FormattedFilteredGenomicData_AlleleFrequencies_NOMAF_",
                   ncol(df_freq),"clones_",nrow(df_freq),"snps.csv"),
            append=FALSE)
...
list_geno[[i]]$df %>% 
  ...
  write_csv(paste0("data/DryadRepo/ListSNPs_",specMAF,"_",nrow(list_geno[[i]]$df),"snps.csv"),append=FALSE)
}
```
...
df <- read.csv("data/DryadRepo/FormattedFilteredGenomicData_AlleleCounts_NOMAF_454clones_9185snps.csv", row.names = 1) 
...
temp_df <- read.csv("data/DryadRepo/SnpCodesMatching.csv") %>% 
...
Percentage of missing data:
  ...
  - in the genomic dataset with MAF: `r round(sum(is.na(list_geno$list_maf$df)) * 100 / (dim(list_geno$list_maf$df)[1]*dim(list_geno$list_maf$df)[2]),2)`%.
  
  - in the genomic dataset without MAF:`r round(sum(is.na(list_geno$list_nomaf$df)) * 100 / (dim(list_geno$list_nomaf$df)[1]*dim(list_geno$list_nomaf$df)[2]),2)`%.
...
setdiff(colnames(list_geno$list_maf$df),prop$clon) # this is ok :)
...
list_geno$list_nomaf$df[1:10,1:10]  %>% kable_mydf(boldfirstcolumn = T, font_size = 11)
```
...
subset <- list_geno$list_nomaf$df[list_geno$list_nomaf$df$max.Q=="Q5",]
set.seed(6)
...
for(i in 1:length(list_geno)){
  ...
  write_csv(paste0("data/DryadRepo/ImputedGenomicData_AlleleFrequencies_",
                   specMAF,"_",
                   nrow(list_geno[[i]]$df),"pops_",ncol(list_geno[[i]]$df[,-1]),"snps.csv"),
            append=FALSE)
}



list_geno$list_nomaf$df[1:10,1:8]  %>% 
  mutate(across(where(is.numeric), round, 2)) %>%
  kable_mydf(boldfirstcolumn = F, font_size = 11)
```
