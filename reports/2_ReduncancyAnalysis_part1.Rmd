...
# my own functions
source("scripts/functions/kable_mydf.R") # building pretty tables for the html
source("scripts/functions/extract_climatedt_metadata.R") # extracting meta data of the climatic variables in ClimateDT
```
...
# ==============================
geno_ind <- read.csv("data/DryadRepo/ImputedGenomicData_AlleleCounts_withoutmaf.csv",row.names = 1) %>% 
  t() %>% 
  as.data.frame()
...
# ===================================
geno_pop <- read.csv("data/DryadRepo/ImputedGenomicData_AlleleFrequencies_withoutmaf.csv",
                     row.names = 1)
...
```{r LoadImtGenDataMAF}
geno_pop_maf <-  read.csv("data/DryadRepo/ImputedGenomicData_AlleleFrequencies_withmaf.csv")
...
pdf(width=10,"figs/RDA/PCAplot.pdf")
par(mfrow=c(1,2))
...
past_clim <- raw_clim %>% 
  ...
  group_by(pop) %>% 
  dplyr::summarise_all(list(mean_ref="mean",sd_ref="sd"))

# z-scores
zscores <- raw_clim %>% 
  dplyr::filter(year=="2041-2070") %>% 
  dplyr::select(pop,year,all_of(x)) %>% 
  dplyr::rename(mean_fut=x) %>% 
  left_join(past_clim,by="pop") %>% 
  dplyr::mutate(zscore=(mean_fut-mean_ref)/sd_ref)# %>% dplyr::select(pop,year,zscore)
...
abs_mean_zscore <- zscores %>% 
  dplyr::select(pop,clim_var,zscore) %>% 
  spread(clim_var,zscore) %>% 
...
p1 <- zscores %>%
  mutate(clim_var=factor(clim_var,levels=c(abs_mean_zscore$variable))) %>% 
  ggplot(aes(x=clim_var, y=zscore)) + 
...
To better visualize to what extent the predicted future climatic values differ from the distribution of the climatic values across the period 1901-1950, we generate `ggridges` plots that show for each climatic variable:

  - its population-specific distributions across the period 1901-1950, colored according to the main gene pool the populations belong to.
  
  - its population-specific predicted mean value across the period 2041-2070.
  
Figures are stored in `RidgelinePlotsClimatePopulations.pdf`

```{r FunctionToPlotggridges, message=F, results="hide", warning=F}
# Function to plot the distribution of the annual climatic values of the populations across the period 1901-1950 +
# their predicted climatic value across the period 2041-270.
make_ggridge_plots <- function(selected_var){

# we extract the meta data from ClimateDT for the selected climatic variables
metadata_var <- extract_climatedt_metadata(var_clim = selected_var)
  
# Annual values of the climatic variables between 1901 and 1950
past_clim <- raw_clim %>% 
  dplyr::select(pop,year,any_of(selected_var)) %>% 
  dplyr::rename(clim_values=selected_var) %>% 
...
# Sort the populations according to the mean climatic value across 1901-1950
mean_clim_value <- past_clim %>% 
  group_by(pop) %>% 
  dplyr::summarise(mean=mean(clim_values)) %>% 
  arrange(mean)

# Extract the predicted mean value of the climatic variable across the period 2041-2070
fut_clim <- raw_clim %>% 
  dplyr::select(pop,year,any_of(selected_var)) %>% 
  dplyr::rename(clim_values=selected_var) %>% 
  dplyr::filter(year=="2041-2070") %>% 
  mutate(pop = factor(pop, levels = mean_clim_value$pop))

# Generate the plot
past_clim %>% 
  mutate(pop = factor(pop, levels = mean_clim_value$pop)) %>% 
  ggplot(aes(x = clim_values, y = pop, fill = mainGPpop)) +
  scale_fill_manual(name = "Main gene pool",
                    values = c("orangered3","gold2","darkorchid3","navyblue","turquoise2","green3")) +
  stat_density_ridges(quantile_lines = TRUE,
                    alpha=0.6,
                    color="gray30") +
  geom_point(aes(x = clim_values, y = pop, color = "red"), data = fut_clim, inherit.aes = F) +
  scale_color_discrete(labels="2041-2070 climate value", name="") +
  theme_minimal() + 
  ylab("") + xlab("") + 
  ggtitle(paste0(metadata_var$description, " (", metadata_var$unit_symbol,") - ",metadata_var$label)) 

# past_clim %>% 
#   mutate(pop = factor(pop, levels = mean_clim_value$pop)) %>% 
#   ggplot(aes(x = clim_values, y = pop, fill = mainGPpop)) +
#   geom_density_ridges(aes(point_color = mainGPpop), 
#                       alpha=0.2,
#                       point_alpha = 1,
#                       point_shape=20,
...
#                     values = c("orangered3","gold2","darkorchid3","navyblue","turquoise2","green3")) +
#   
#   scale_discrete_manual(name="Main gene pool", aesthetics = "point_color", values = c("orangered3","gold2","darkorchid3","navyblue","turquoise2","green3")) +
#   theme_ridges() + 
#   theme_minimal() + 
#   #theme(legend.position = "none") +
#   geom_point(aes(x = clim_values, y = pop, color = "red"), data = fut_clim, inherit.aes = F) +
...
pdf(width = 9, height = 9, "figs/ExploratoryAnalyses/RidgelinePlotsClimatePopulations.pdf")
lapply(var_clim, make_ggridge_plots)
dev.off()
```
...
`AHM` is calculated as follows: `AHM`= (`bio1`+10)/(`bio12`/1000)) with `bio1` the annual mean temperature and `bio12` the annual precipitation.

We look at the values of `AHM`, `bio1` and `bio12` for the two populations from North Africa in the table below. `mean_ref` and `sd_ref` are the mean and standard deviation of the distribution of the climatic variables across the reference period 1901-1950. `mean_fut` is the predicted mean value of the climatic variable for the period 2041-2070. `zscore` is the z-score calculated as follows: `zscore` = (`mean_fut` - `mean_past` ) / `sd_ref`.

```{r TableAHMValues}
zscores %>% 
  dplyr::filter(clim_var %in% c("bio1","bio12","AHM")) %>% 
  dplyr::filter(mainGPpop %in% c("Northern Africa")) %>% 
  dplyr::select(-year) %>% 
  kable_mydf()
```

We do the same for `SHM` which is calculated as follows: `SHM` = `MWMT`/(`SP`/1000) with `MWMT` the mean warmest month temperature and `SP` the summer precipitation.

```{r TableSHMValues}
zscores %>% 
  dplyr::filter(clim_var %in% c("MWMT","SP","SHM")) %>% 
  dplyr::filter(mainGPpop %in% c("Northern Africa")) %>% 
  dplyr::select(-year) %>% 
  kable_mydf()
```
...
```{r StepwiseSelectionPopLevel, eval=T}
...
```{r VartPartIndividualLevel, eval=T}
...
```{r VartPartPopLevelCoordinates, eval=T}
list_sum_var_part <- lapply(list(yearly_var,drought_var,set3_var), function(x){
...
```{r RunRDAmodels, eval=T, results="hide"}
