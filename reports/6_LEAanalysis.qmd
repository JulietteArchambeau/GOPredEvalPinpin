...
list_g_gap <- lapply(clim_dfs[[2]], function(clim_fut){
...
list_g_gap <- lapply(list_g_gap, function(x){

x$g_gap_allsnps <- genetic.gap(input = geno,
                               env = clim_past,
                               pred.env = x$clim_fut,
                               K = 5)
...
Delta = clim_past - x$clim_fut  

x$dist_env = sqrt( rowSums(Delta^2) )
...
gps <- readRDS(here("data/GenomicData/MainGenePoolInformation.rds"))[[2]]
```
...
lapply(list_g_gap, function(y){

cor(unique(y$g_gap_allsnps$offset),unique(y$dist_env^2))
...
barplot(list_g_gap[[1]]$g_gap_allsnp$eigenvalues, col = "orange", xlab = "Axes", ylab = "Eigenvalues") 
```
...
lapply(list_g_gap, function(x){

  make_eucli_plot(X = x$dist_env,
          Y = sqrt(x$g_gap_allsnp$offset),
          colors = gps$color_main_gp_pop, 
          color_names = gps$main_gp_pop,
          ylab = "sqrt(genetic gap)",
          plot_title = paste0("All SNPs - ",x$gcm))  
})
```
...
list_snpsets <- list(candidates = list(set_name = "Candidate SNPs",
...
list_g_gap <- lapply(list_g_gap, function(y){
  
y$g_gap_snpsets <- lapply(list_snpsets, function(x) {
...
lapply(y$g_gap_snpsets, function(x) make_eucli_plot(
  X = y$dist_env,
  Y = sqrt(x$g_gap$offset),
  colors = gps$color_main_gp_pop,
  color_names = gps$main_gp_pop,
  ylab = "sqrt(genetic gap)",
  plot_title = paste0(x$set_name," - ",y$gcm)))
...
lapply(list_g_gap[[1]]$g_gap_snpset, function(x) 
...
lapply(list_g_gap, function(y){
  lapply(y$g_gap_snpsets, function(x){
    
cor(unique(x$g_gap$offset),unique(y$dist_env^2))
