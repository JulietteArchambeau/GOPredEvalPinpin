...
clim_past <- genotypes %>% 
  ...
  left_join(clim_dfs$clim_past, by="pop") %>% 
...
clim_fut <- genotypes %>% 
  mutate(pop = str_sub(clon,1,3)) %>% 
  left_join(clim_dfs$clim_fut, by="pop") %>% 
  dplyr::select(-pop,-clon)
```
...
saveRDS(gp_poplevel, here("data/GenomicData/MainGenePoolPopulations.rds"))
...
lapply(list_ggap, function(x) make_eucli_plot(
  X = dist_env,
