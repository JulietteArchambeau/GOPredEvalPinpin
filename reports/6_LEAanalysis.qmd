...
geno <-  read.csv(here("data/DryadRepo/FormattedFilteredGenomicData_AlleleCounts_MAF_454clones_9879snps.csv")) %>% 
...
geno <-  read.csv(here("data/DryadRepo/ImputedGenomicData_AlleleCounts_NOMAF_454clones_9185snps.csv")) %>% 
  column_to_rownames("snp_ID") %>% 
  t() %>% 
  as_tibble()
```
...
```{r PlotGeneticGapVsClimDistance}
# Gene pool colors
...
plot(dist_env, g_gap$distance, xlab ="Euclidean distance",  ylab ="sqrt(genetic gap)", 
     cex = 1, col = genotypes$color_main_gp,
     pch=16)
legend("topleft", legend=unique(genotypes$main_gp),
       bty="n",
       pch=19, 
       cex=1, col=unique(unique(genotypes$color_main_gp)),
       ncol=1,
