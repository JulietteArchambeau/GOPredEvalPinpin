...
geno <-  read.csv(here("data/DryadRepo/ImputedGenomicData_AlleleCounts_withmaf.csv")) %>% 
  column_to_rownames("snp_ID") %>% 
  t()

# We run LFMM 
mod_withmaf <- lfmm2(input = geno,
...
geno_corrected <- as.matrix(clim_past) %*% t(mod_withmaf@B) %>% 
  set_rownames(rownames(geno)) %>% 
  as.data.frame()

# save the matrix for future analyses
