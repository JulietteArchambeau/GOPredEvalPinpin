...
We compare the genomic offset predictions from the four following methods:
  ...
  - Generalized dissimilarity modelling (GDM). See report `to fill`.
  
  - Latent factor mixed model (LFMM) . See report `to fill`.
  
  - Gradient Forest (GF). See report `to fill`.
  
  - Redundancy analysis (RDA). See report `to fill`.
