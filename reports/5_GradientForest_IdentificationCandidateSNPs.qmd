...
list_outliers <- list("RDA" = rda_outliers[[3]]$outliers$snp,
                      ...
                      "GF (uncorrected)" = gf_out$gf_raw$outliers_rank005,
                      "GF (corrected)" = gf_out$gf_X$outliers_rank005)
...
Outlier SNPs identified across independent runs are not exactly the same. 

The Venn diagrams below show the number of SNPs in common across 3 independent runs (for GF models not corrected for population structure).
 ...
 **<span style="color: red;">For the following analyses, we keep the outlier SNPs that were among the 0.5% SNPs with the lowest empirical $p$-values in each of the three independent runs:</span>**

```{r CommonCandidates}
cand <- list("run 1"=gf_out_1$gf_raw$outliers_rank005,
             "run 2"=gf_out_2$gf_raw$outliers_rank005,
             "run 3"=gf_out$gf_raw$outliers_rank005) %>% 
  Reduce(intersect, .)

saveRDS(cand, here::here("outputs/GF/gf_common_outliers_across_three_runs.rds"))
```

So, at the end, we identified `r length(cand)` outlier SNPs with the GF-based GEA.
