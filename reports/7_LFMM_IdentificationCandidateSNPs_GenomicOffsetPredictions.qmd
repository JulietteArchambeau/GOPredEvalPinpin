...
Below, we will estimate the genetic gap for:
  ...
  - the two sets of candidate SNPs (see report `8_GeneratingSNPsets.qmd`).
  
  - the set of control SNPs
  
  - all SNPs (as advised by the developers of the `genetic.gap` function) 

In the paper (and in the validation steps), only one of the two sets of candidate SNPs is used: the one with candidate SNPs identified by at least one GEA method among *RDA*, *pRDA*, *LFMM*, *BayPass* and *GF*.

## Loading future climatic data
...
## GO predictions using all SNPs

We run the `genetic.gap` function.

```{r EstimateGeneticGapAllSNPs}
...
```{r CorrelationEuclideanDistanceGeneticGapAllSNPs}
...
```{r PlotAllSNPsEucliDistance, fig.width=12, results="hide"}
par(mfrow=c(1,2))
...
```{r MakeEucliPlotsGGPLOTallsnps, warning=F}
...
```{r MakeGomapsAllsnps, fig.width=12, fig.height=7, results="hide", warning=F}
...
ggsave(here("figs/LFMM/GOMaps_PopLocations_AllSNPs_SI.png"), go_maps, width=10,height=6)
...
```{r GOpredNFIplotsWithAllSNPs, fig.width=8,fig.height=8}
...
ggsave(here("figs/LFMM/NFI_GOmap_AllSNPs_SI.png"), p_SI, width=8,height=8)
p
```
...
```{r GOpredictionsCGallSNPs, fig.height=6,fig.width=12}
...
ggsave(filename = here(paste0("figs/LFMM/GOmap_allSNPs_",cg_name,"_SI.png")), p_SI, width=7,height=5)
p
})
...
## GO predictions using sets of SNPs
...
if(x$set_code=="cand"){ 
  ...
  ggsave(here("figs/LFMM/GOMaps_PopLocations_CandidateSNPs_SI.png"), go_maps, width=10,height=6)
    ...
    ggsave(here("figs/LFMM/GOMaps_PopLocations_ControlSNPs_SI.png"), go_maps, width=10,height=6)
}
...
if(x$set_code=="cand"){ 
  p_SI <- p + theme(plot.title = element_blank())
  ggsave(here("figs/LFMM/NFI_GOmap_CandidateSNPs_SI.pdf"), p_SI, width=6,height=6, device="pdf")
  ggsave(here("figs/LFMM/NFI_GOmap_CandidateSNPs_SI.png"), p_SI, width=6,height=6)
  
  } else if(x$set_code=="control"){
    p_SI <- p + theme(plot.title = element_blank())
    ggsave(here("figs/LFMM/NFI_GOmap_ControlSNPs_SI.pdf"), p_SI, width=6,height=6, device="pdf")
    ggsave(here("figs/LFMM/NFI_GOmap_ControlSNPs_SI.png"), p_SI, width=6,height=6)
    }
...
p <- lapply(snp_sets, function(x) {
 ...
 if(x$set_code=="cand") {
   ...
   p_SI %>% 
   ggsave(filename = here(paste0("figs/LFMM/GOmap_",x$set_code,"_",cg_name,"_SI.png")), width=5,height=5)
  ...
  if(x$set_code=="control") {
   ...
   p_SI %>% 
   ggsave(filename = here(paste0("figs/LFMM/GOmap_",x$set_code,"_",cg_name,"_SI.png")), width=7,height=5)
