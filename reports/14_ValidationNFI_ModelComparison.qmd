---
title: "Comparing the estimation accuracy of Bayesian mortality models in the NFI plots"
...
We load the NFI data in the DRYAD repository.

```{r LoadNFIData}
data <- read_csv(here("data/DryadRepo/NFIdata_cleaned.csv"),show_col_types = FALSE)
```


# Introduction

**Goal:** determining which mathematical model will be used to estimate the association between genomic offset predictions and mortality rates in the NFI plots.
...
lapply(1:nb_simulations, function(x) run_randomGO(stancode,data)) %>% saveRDS(here(paste0("outputs/ValidationNFI/Simulations/",nb_simulations,"simulations_m3_truedata_randomGO.rds")))
```
...
df <- readRDS(file=here(paste0("outputs/ValidationNFI/Simulations/",nb_simulations,"simulations_m3_truedata_randomGO.rds"))) %>% 
  bind_rows(.id="sim_ID") %>% 
...
## M5_1: adding spatial autocorrelation
...
We then simulate the observed counts of dead trees, with a balanced sampling design (i.e., 30 sampled trees per plot).
...
mod %>% saveRDS(file=here("outputs/ValidationNFI/Simulations/1simulation_m5_1_balanceddesign.rds"))
```


```{r ModelCoeffM51}
mod <- readRDS(file=here("outputs/ValidationNFI/Simulations/1simulation_m5_1_balanceddesign.rds"))
...
```{r ComparingTrueVsEstimatedCovarianceKernel}
...
make_coeff_table(mod,
  ...
  bind_rows(.id = "nb_sim") %>% 
  saveRDS(file=here(paste0("outputs/ValidationNFI/Simulations/",nb_simulations,"simulations_m3_balanceddesign_withspatialautocorrelation.rds")))
```
...
readRDS(file=here(paste0("outputs/ValidationNFI/Simulations/",
                         nb_simulations,"simulations_m3_balanceddesign_withspatialautocorrelation.rds"))) %>% 
  calc_coverage_across_sims %>% 
  kable_mydf()
```
...
lapply(1:nb_simulations, function(x) run_randomGO(stancode,data)) %>% saveRDS(here(paste0("outputs/ValidationNFI/Simulations/",nb_simulations,"simulations_m6_2_truedata_randomGO.rds")))
```
...
# Extract the outputs and identify the simulations in which the 95% and 80% credible intervals overlap with zero. 
df <- readRDS(file=here(paste0("outputs/ValidationNFI/Simulations/",nb_simulations,"simulations_m6_2_truedata_randomGO.rds"))) %>% 
  bind_rows(.id="sim_ID") %>% 
  dplyr::filter(term == "beta_GO") %>%
  mutate(conf80_include_zero = if_else(conf80_low<=0 & conf80_high>=0, TRUE,FALSE),
         conf95_include_zero = if_else(conf95_low<=0 & conf95_high>=0, TRUE,FALSE))
