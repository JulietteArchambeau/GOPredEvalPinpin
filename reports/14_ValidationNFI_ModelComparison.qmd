...
\begin{align*}
m_i &\sim \text{Binomial}(N_i,p_i)\\
\text{log}(-\text{log}(1-p_i)) &= \beta_{0,c} +  \beta_{C,c} C_i + \beta_{GO,c} GO_i + \text{log}(\Delta_i) \\
\end{align*}

with $\beta_{0,c}$ the country-specific intercept, $C_{i}$ the basal area of all trees (all species confounded) in the plot $i$ (to account for the competition between trees) and its associated country-specific slope $\beta_{C,c}$, $GO_{i}$ the estimated genomic offset in the plot $i$ and its associated country-specific slope $\beta_{GO,c}$.
...
<span style="color: red;"> Problem: this model is already long to fit on 200 simulated spatial points, so it would not be possible to fit it on the `r nrow(data)` NFI plots... But then, how can we account for spatial autocorrelation?<span/>
...
# Extract the 95% credible intervals and the median
  ...
  broom.mixed::tidyMCMC(mod,
    ...
    dplyr::rename(conf80_low=conf.low,
     ...
     dplyr::select(term,mean,median,std_error,conf80_low,conf80_high,conf95_low,conf95_high, rhat,ess) %>% 
...
```{r StanCodeM0}
# Stan code
stancode = stan_model(here("scripts/StanModels/simulations/mo.stan"))
print(stancode)
```
...
# Build coeff table
make_coeff_table(mod,true_params = c(beta_0,beta_GO))

}) %>% 
  bind_rows(.id = "nb_sim") %>% 
  saveRDS(file=here("outputs/ValidationNFI/Simulations/sim_m0.rds"))
```

```{r TableCoeffM0}
readRDS(file=here("outputs/ValidationNFI/Simulations/sim_m0.rds")) %>% 
...
```{r StanCodeM1}
# Stan code
stancode = stan_model(here("scripts/StanModels/simulations/m1.stan"))
print(stancode)
```
...
make_coeff_table(mod,true_params = c(beta_0,beta_GO))
  ...
  bind_rows(.id = "nb_sim") %>% 
  saveRDS(file=here("outputs/ValidationNFI/Simulations/sim_m1.rds"))
```

```{r TableCoeffM1}
readRDS(file=here("outputs/ValidationNFI/Simulations/sim_m1.rds")) %>% 
  calc_coverage_across_sims %>% 
  kable_mydf()
```
...
```{r StanCodeM2}
# Stan code
stancode = stan_model(here("scripts/StanModels/simulations/m2.stan"))
print(stancode)
```
...
# Build coeff table
make_coeff_table(mod,true_params = c(beta_0,beta_GO,beta_C))

}) %>% 
  bind_rows(.id = "nb_sim") %>% 
  saveRDS(file=here("outputs/ValidationNFI/Simulations/sim_m2_balanceddesign.rds"))
```

```{r TableCoeffM2BalancedDesign}
readRDS(file=here("outputs/ValidationNFI/Simulations/sim_m2_balanceddesign.rds")) %>% 
  calc_coverage_across_sims %>% 
  kable_mydf()
```
...
make_coeff_table(mod,true_params = c(beta_0,beta_GO,beta_C))
  ...
  bind_rows(.id = "nb_sim") %>% 
  saveRDS(file=here("outputs/ValidationNFI/Simulations/sim_m2_unbalanceddesign.rds"))
```


```{r TableCoeffM2UnbalancedDesign}
readRDS(file=here("outputs/ValidationNFI/Simulations/sim_m2_unbalanceddesign.rds")) %>% 
  calc_coverage_across_sims %>% 
  kable_mydf()
```
...
make_coeff_table(mod,true_params = c(beta_0,beta_GO,beta_C))
  ...
  bind_rows(.id = "nb_sim") %>% 
  saveRDS(file=here("outputs/ValidationNFI/Simulations/sim_m2_truedesign.rds"))
```

```{r TableCoeffM2TrueDesign}
readRDS(file=here("outputs/ValidationNFI/Simulations/sim_m2_truedesign.rds")) %>% 
  calc_coverage_across_sims %>% 
  kable_mydf()
```
...
```{r StanCodeM3}
# Stan code
stancode = stan_model(here("scripts/StanModels/simulations/m3.stan"))
print(stancode)
```
...
nb_years <- sample(5:15, N, replace=T) # random time periods between inventory dates (between 5 and 15 years)
beta_0 <- c(runif(1,-7.5,-6.5),runif(1,-6.5,-5.5)) # country-specific intercepts
beta_GO <- runif(1,0.25,0.75) # coeff of the genomic offset
beta_C <- runif(1,0.5,1.5) # coeff of the basal area
GO <- rnorm(N,0,1) # genomic offset
GO <- (GO - mean(GO)) / sd(GO) # scaling the genomic offset
C <- rnorm(N,0,1) # basal area
C <- (C - mean(C)) / sd(C) # scaling the basal area
eta <- beta_0[country] + beta_GO * GO + beta_C * C + log(nb_years) # linear predictor
p <- 1-exp(-exp(eta)) # probability of mortality in each plot
nb_dead <- rbinom(N,nb_tot, p) # nb of dead trees in each plot
...
make_coeff_table(mod,true_params = c(beta_0,beta_GO,beta_C))
  ...
  bind_rows(.id = "nb_sim") %>% 
  saveRDS(file=here("outputs/ValidationNFI/Simulations/sim_m3_balanceddesign.rds"))
```

```{r TableCoeffM3BalancedDesign}
readRDS(file=here("outputs/ValidationNFI/Simulations/sim_m3_balanceddesign.rds")) %>% 
  calc_coverage_across_sims %>% 
  kable_mydf()
```
...
make_coeff_table(mod,true_params = c(beta_0,beta_GO,beta_C))
  ...
  bind_rows(.id = "nb_sim") %>% 
  saveRDS(file=here("outputs/ValidationNFI/Simulations/sim_m3_unbalanceddesign.rds"))
```


```{r TableCoeffM3UnbalancedDesign}
readRDS(file=here("outputs/ValidationNFI/Simulations/sim_m3_unbalanceddesign.rds")) %>% 
  calc_coverage_across_sims %>% 
  kable_mydf()
```
...
```{r 100SimulationsM3TrueDesign, eval=F}
set.seed(49205)
nb_setseed <- sample(1:1000,nb_simulations,replace=FALSE)

simul_data <- function(x, data, stancode){
...
beta_0 <- runif(2,min=-8,max=-5) # country-specific intercepts
...
broom.mixed::tidyMCMC(mod,
     ...
     dplyr::rename(std_error=std.error) %>% 
     dplyr::select(term,mean,median,std_error,conf80_low,conf80_high,conf95_low,conf95_high, rhat ,  ess) %>% 
  mutate(true_values = c(beta_0[1],beta_0[2],beta_C,beta_GO)) %>% 
  mutate(conf80_coverage = if_else(conf80_low<=true_values & conf80_high>=true_values, TRUE,FALSE),
         conf95_coverage = if_else(conf95_low<=true_values & conf95_high>=true_values, TRUE,FALSE))
...
lapply(nb_setseed, function(x) simul_data(x=x,data=data, stancode=stancode)) %>% 
  saveRDS(here("outputs/ValidationNFI/Simulations/m3_truedesign_100simulations.rds"))
```

```{r Output100SimulationsM3TrueDesign}
readRDS(file=here("outputs/ValidationNFI/Simulations/m3_truedesign_100simulations.rds")) %>% 
  bind_rows(.id="sim_ID") %>% 
  group_by(term) %>% 
  group_split() %>% 
  purrr::map(\(x){
...
#### Random genomic offset

```{r SimulationsM4TrueData, eval=F}
...
lapply(1:2, function(x) run_randomGO(stancode,data)) %>% saveRDS(here("outputs/ValidationNFI/Simulations/m3_truedata_randomGO_100simulations.rds"))
```

```{r TableCoeffM4TrueData, eval=F}
readRDS(file=here("outputs/ValidationNFI/Simulations/m3_truedata_randomGO_100simulations.rds")) %>% 
  bind_rows(.id="sim_ID") %>% 
...
```{r StanCodeM51}
# Stan code
stancode = stan_model(here("scripts/StanModels/simulations/m5_1_noncentered.stan"))
print(stancode)
```
...
mod %>% saveRDS(file=here("outputs/ValidationNFI/Simulations/m5_1_init_random.rds"))
```


```{r ModelCoeffM51}
#mod <- readRDS(file=here("outputs/ValidationNFI/Simulations/m5_1_init_random.rds"))
mod <- readRDS(file=here("outputs/ValidationNFI/Simulations/m5_1_init0.rds"))
...
# Stan code
stancode = stan_model("scripts/StanModels/simulations/m4.stan")
print(stancode)
