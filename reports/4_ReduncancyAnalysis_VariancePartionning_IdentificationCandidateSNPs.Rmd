...
clim <- readRDS("data/ClimaticData/MaritimePinePops/ClimatePopulationLocationPointEstimates_ReferencePeriods_noADJ.rds")[[1]]$ref_means %>% 
...
gps <- readRDS("data/GenomicData/MainGenePoolInformation.rds")
```
...
Knowing that, the set of selected climatic variables has to capture the changes in climatic conditions that will threaten the maritime pine population viability. In the context of climate change and in the Mediterranean area, it is necessary to include climatic variables capturing the summer drought conditions. The set of climatic variables has also to capture the climatic conditions that drive population vulnerability (decrease in fitness) in the different evaluation part, which may be driven by different climatic variables that the climatic variables that will drive population vulnerability in natural conditions. Therefore, one option may be not to select the same climatic variables for the different study goals and evaluation methods, which is discussed below.
...
Ideally, for this evaluation part, the genomic offset has to be estimated on a set of variables incorporating both summer drought conditions and mean climatic conditions across the whole year. (ie *yearly-climate genomic offset*, *yearly-climate GO*).
...
However, using different sets of climatic variables adds a supplementary level of complexity to the paper, which is already very dense. So, we decided to use one set of climatic variables capturing both the summer drought conditions and the mean annual conditions. The drawback of this approach is that if the variables capturing the summer-drought conditions are not important to explain the gene-environment relationships, the genomic offset approach will not be able to capture population vulnerability to drought conditions, which is problematic in some of the evaluation parts (see \@ref(ClimVarEvalPart))
...
Outliers can be identified based on their *extremeness* along a *distribution of Mahalanobis distances* estimated between each locus and the center of the RDA space using a certain number of axes (K). The Mahalanobis distances are calculated with the `rdadapt` function from @capblancq2021redundancy, which returns $p$-values and $q$-values for each SNP. Different thresholds can then be used to identify the outliers:
  ...
  -  @capblancq2020climate uses a $p$-value threshold with a Bonferroni correction to account for multiple testing: $p$-value < 0.01 / number of SNPs (see [code](https://github.com/Capblancq/Local-Adaptation-Fagus-sylvatica)). In our study, this threshold results in selecting 16 SNPs with the RDA-raw model, and 5 with the RDA-X model (and 1 in common between the two methods). 

  - @capblancq2021redundancy ([code](https://github.com/Capblancq/RDA-landscape-genomics)) rank-ordered the SNPs based on their $p$-values (the $p$-value of a given SNP captures how far its Mahalanobis distance is from the distribution of Mahalanobis distances of all SNPs) and identified the 0.2% of the SNPs with the lowest $p$-values. In our study, this threshold results in selecting 19 SNPs (0.002 * `r dim(geno_pop)[[2]]`=`r 0.002*dim(geno_pop)[[2]]`). We will use this threshold in the following analyses.
...
# P-values are rank-ordered and we select the 0.2% with the lowest p-values
outliers_maha <- GSout %>% 
  arrange(pvalues) %>% 
  slice(1:(0.002*nrow(.))) %>% 
  mutate(maha_meth = TRUE) %>% 
...
```{r ExportClimNames}
set2_var$variables %>% saveRDS("data/ClimaticData/NamesSelectedVariables.rds")
```
