...
```{r StandardizeClimaticVariables}
## Standardization of the climatic variables
clim.sc <- clim

clim.sc2 <- clim %>% 
  dplyr::select(-c("pop","latitude","longitude","elevation")) %>% 
  scale(center=TRUE, scale=TRUE)

## Recovering scaling coefficients
scale_clim <- attr(clim.sc2, 'scaled:scale')
center_clim <- attr(clim.sc2, 'scaled:center')

clim.sc[,colnames(clim.sc2)]  <-  clim.sc2
```
...
df.var <- read.csv("data/DryadRepo/PopulationStructureCorrea2015.csv") %>% 
  ...
  left_join(clim.sc,by="pop") %>% # merge with the climatic variables
...
rda.full <- rda(geno ~ bio1 + bio2 + bio3 + bio4 + bio5 + bio6 + bio7 + bio8 + bio9 + 
                  ...
                  NFFD + DMA + EPQ + RMT + AHM + SHM + TD + EMT + eFFP + FFP + PAS, df.var)
```
...
drought.var <- c("bio17","MWMT")

clim %>%
  dplyr::select(all_of(drought.var)) %>%
  unique() %>%
...
global.var <- c("bio1","bio12","bio17","MWMT","MCMT")

clim %>%
  dplyr::select(all_of(global.var)) %>%
  unique() %>%
  pairs.panels(scale=T,hist.col="palegreen1")
```
...
<!-- Create a df with all variables scaled and one with none! -->
