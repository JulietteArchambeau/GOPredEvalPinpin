...
# ==============================
geno <- read.csv("data/DryadRepo/ImputedGenomicData_AlleleCounts_NOMAF_454clones_9817snps.csv",row.names = 1) %>% 
  t() %>% 
  as.data.frame()
...
# ===================================
genofreq <- read.csv("data/DryadRepo/ImputedGenomicData_AlleleFrequencies_NOMAF_34pops_9817snps.csv",
                     row.names = 1)
...
The genomic dataset with *individual-based allele counts* contains `r nrow(geno)` clones (i.e. genotypes) and `r ncol(geno)` SNPs. The genomic dataset with *population-based allele frequencies* contains `r nrow(genofreq)` populations and `r ncol(genofreq)` SNPs.
...
We can look at the distribution of the variables.
...
png(file = "figs/ExploratoryAnalyses/CorrplotClimVariables.png",width=1000,height=1000,res=100)
...
df.var.ind <- read.csv("data/DryadRepo/PopulationStructureCorrea2015.csv") %>% 
...
# identical(as.vector(df.var.ind$clon),rownames(geno)) # should be TRUE 

df.var.ind[1:10,1:14] %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)

# We create a df with standardized variables (ie centered and scaled)
df.var.ind.sc <- df.var.ind %>% 
  dplyr::mutate(across(where(is.numeric), ~ (. - mean(.)) / sd(.)))

df.var.ind.sc[1:10,1:14] %>%
  kable() %>%
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F)
```
...
```{r LoadImtGenDataMAF}
genomaf <-  read.csv("data/DryadRepo/ImputedGenomicData_AlleleFrequencies_MAF_34pops_10520snps.csv")
...
rda.null <- rda(geno ~ 1, df.var.ind.sc)
```
...
rda.full <- rda(geno ~ bio1 + bio2 + bio3 + bio4 + bio5 + bio6 + bio7 + bio8 + bio9 + 
                  ...
                  NFFD + DMA + EPQ + RMT + AHM + SHM + TD + EMT + eFFP + FFP + PAS, df.var.ind.sc)
```
...
saveRDS(mod,file="outputs/VariableSelection/StepwiseSelectionModel_GenotypeLevel.rds")
```

```{r OutputsStepwiseSelectionProcedure, message=FALSE, eval=TRUE, echo=FALSE}
mod <- readRDS("outputs/VariableSelection/StepwiseSelectionModel_GenotypeLevel.rds")
mod
mod$anova
```
...
saveRDS(mod,file="outputs/VariableSelection/ForwardSelectionModel_GenotypeLevel.rds")
```

```{r OutputsForwardSelectionProcedure, message=FALSE, eval=TRUE, echo=FALSE}
mod <- readRDS("outputs/VariableSelection/ForwardSelectionModel_GenotypeLevel.rds")
mod
mod$anova
```
...
```{r StepwiseSelectionPopLevel}
clim.sc <- clim %>% dplyr::mutate(across(where(is.numeric), ~ (. - mean(.)) / sd(.)))

rda.null <- rda(genofreq ~ 1, clim.sc)

rda.full <- rda(genofreq ~ bio1 + bio2 + bio3 + bio4 + bio5 + bio6 + bio7 + bio8 + bio9 + 
                  bio10 + bio12 + bio13 + bio14 + bio15 + bio16 + bio17 + bio18 + 
                  bio19 + Eref + CMD + MCMT + MSP + MWMT + AOP + GDD5 + GDD18 + 
                  NFFD + DMA + EPQ + RMT + AHM + SHM +  EMT + eFFP + PAS, clim.sc)


## Stepwise procedure with ordiR2step function
mod <- ordiR2step(rda.null, rda.full, Pin = 0.01, R2permutations = 1000, R2scope = T)
saveRDS(mod,file="outputs/VariableSelection/StepwiseSelectionModel_PopLevel.rds")
```

```{r OutputsStepwiseSelectionProcedure, message=FALSE, eval=TRUE, echo=FALSE}
mod <- readRDS("outputs/VariableSelection/StepwiseSelectionModel_PopLevel.rds")
mod
mod$anova
```
